...
### 교과서를 베끼지 않는다 — 종속성과 표절을 함께 막는 세 가지

강의노트는 **웹에 공개**된다. 특정 교과서를 그대로 옮기면 저작권 문제가 되고,
그 교과서를 쓰지 않는 사람에게는 읽을 수 없는 자료가 된다.

**1. 교과서 고유 번호를 화면에 노출하지 않는다.**
...
`[표 Ⅱ-9]`, `[그림 Ⅱ-21]` 같은 번호는 **그 교과서 안에서만 뜻이 통한다.**
캡션은 **무엇을 담은 표인지**로 쓴다.

**2. 데이터의 원출처를 반드시 밝힌다.**

수치 자체는 저작물이 아니다. 교과서가 아니라 **원출처**를 적으면 저작권 문제와
종속성이 동시에 사라진다.
...
**3. 정의 문장을 그대로 옮기지 않는다.**

**개념은 저작물이 아니지만 표현은 저작물이다.** 교과서 문장을 인용 블록
(`fa-quote-left`)에 그대로 넣는 것이 가장 위험하다. 뜻을 지키되 **내 문장으로 다시 쓴다.**

```
교과서: "탐색적 데이터 분석은 통계적 방법과 시각적 표현을 포함한 다양한 방법을
        사용하여 데이터를 자세히 관찰하고 분석하여 … 이해하는 과정이다."
다시 쓰기: "통계와 시각화를 함께 써서 데이터를 여러 각도로 들여다보고,
           그 데이터가 어떤 성질과 짜임새를 가졌는지 알아내는 과정입니다."
```

- **진짜 인용이 필요한 것만 인용한다.** 존 튜키의 말처럼 **인물의 발언**은
  짧게 따고 **누가 한 말인지 밝힌다.** 이건 정당한 인용이다.
- 교과서가 만든 **분류·순서·예시 구성**은 따라도 되지만, **문장은 새로 쓴다.**
- (2026-08-05 사용자 지적. 데이터과학 11개 파일에 교과서 번호 24곳, 축자 인용 22곳이
  들어가 있었다.)
