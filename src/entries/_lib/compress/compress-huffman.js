 ...
 *
 * 앞의 둘과 줄이는 자리가 다르다. 런 렝스와 키워드는 **반복되는 «덩어리»**를 줄이는데,
 ...
 * ---------------------------------------------------------------
 *
 * `ABABABABAB`은 A와 B가 다섯 번씩이다. 트리를 지어 보면 둘 다 한 비트를 받아
 * **압축률이 0%**로 나온다. 고장처럼 보이지만 이것이 허프만의 성질이다 —
 * 짧은 코드를 줄 «자주 나오는 글자»가 있어야 이득이 생긴다.
 *
 * **다만 「횟수가 고르면 이득이 없다」로 뭉뚱그리면 틀린다.** 그 말이 참인 것은
 * **글자 가짓수가 2의 거듭제곱일 때뿐**이다. 가짓수가 3이면 같은 길이 코드가 2비트인데
 * 허프만은 1·2·2비트를 주어 줄어든다. 프리셋 가운데 둘이 실제로 그런 경우라,
 * 화면 문장에 조건을 달지 않으면 **바로 그 아래 계수기가 그 문장을 뒤집는다.**
 *
 ...
 *
 * ---------------------------------------------------------------
 * **같은 글은 언제나 같은 트리가 되어야 한다**
 * ---------------------------------------------------------------
 *
 * 가장 작은 둘을 고를 때 값이 같으면 어느 쪽을 집어도 «압축된 길이»는 같다. 그런데
 * 트리 «모양»은 달라진다. 학생이 두 번 돌려 다른 그림을 보면 그것부터 물어보게 되고,
...
/** 트리 한 노드. 단말 노드는 `ch`를 들고, 내부 노드는 `left`·`right`를 든다. */
 ...
 * 트리를 읽는 방향(왼쪽이 0)과도 결이 맞는다 — **왼쪽으로 갈수록 자주 나오는 글자다.**
...
/** 트리에서 코드표를 읽어 낸다. 왼쪽이 0, 오른쪽이 1. */
export function codesOf(root) {
    const codes = new Map();
    if (!root) return codes;
    /* **단말 노드가 하나뿐인 트리**는 길을 걸을 수가 없어 코드가 빈 문자열이 된다.
...
/** 트리만 짓는다. 화면 없이 값을 얻을 때 쓴다 — 검사와 나란히 비교가 이것을 부른다. */
...
export function huffmanEncode(text, opts = {}) {
    ...
    while (forest.length > 1) {
        ...
        rec.carry({forest: forest.map(cloneTree), root: null, codes: null, merged: merged.seq})
            .say(`둘을 묶어 <b>${merged.n}</b>짜리 노드를 만듭니다. `
...
/** 스냅샷에 실을 사본. **참조로 실으면 다음 장이 앞 장을 고쳐 버린다** — 트리는 계속 자란다. */
...
/** 되돌리기. **트리를 걸어 내려간다** — 검사가 이것으로 원본과 대조한다. */
