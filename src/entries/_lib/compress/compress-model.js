 ...
 *
 * **다른 것은 「한 판」의 뜻이다.** 찾기는 「연산 한 번이 한 판」이라 앞 판이 끝난 상태를
 * 물려받았다. 압축은 물려받을 것이 없다 — **글 하나를 통째로 줄이는 것이 한 판**이고,
 ...
 *
 * **기준(줄이기 전)은 글자마다 같은 길이의 코드를 줬을 때다.** 글자가 k가지면
...
/** 입력 길이 천장. 넘으면 나무가 화면을 벗어나고, 한 판이 길어져 되감아 볼 수가 없다. */
...
/** 글자별 나온 횟수. **나온 차례를 지킨다** — 같은 횟수끼리의 차례가 흔들리면
 *  허프만 나무가 판마다 달라져, 학생이 두 번 돌렸을 때 다른 그림을 본다. */
...
export function createCompressRecorder(text, opts = {}) {
    ...
    const rec = {
        ...
        /** 방법마다 다른 덤을 갈아 끼운다. 허프만이 숲과 나무를 여기 싣는다. */
