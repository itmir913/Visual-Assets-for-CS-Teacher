 ...
 *
 * 탐색 시뮬레이터의 등록부와 같은 자리다. 탭을 늘리거나 프리셋을 바꿀 때
 * 손대는 파일이 하나로 모인다.
 */
 ...
 * `check -- sim compress`가 그 조건을 값으로 지킨다.
 */
export const COMPRESS_PRESETS = [
    {
        ...
        name: '이어진 반복이 많다',
        text: 'AAAAABBCCC',
        hint: '같은 글자가 붙어 있습니다. 런 렝스가 노리는 모양입니다.',
    },
    {
        id: 'flat',
        name: '이어지지 않고 반복된다',
        text: 'ABABABABAB',
        hint: '반복이 분명히 있는데 <b>이어져 있지 않습니다.</b> 런 렝스는 늘어나고 허프만은 그대로입니다.',
    },
    {
        id: 'none',
        name: '반복이 하나도 없다',
        text: 'ABCDEFGHIJ',
        hint: '이어진 반복이 없어 <b>런 렝스에서 압축률이 음수로 나옵니다.</b> '
            + '그런데 허프만은 줄입니다 — <b>「반복이 없다」와 「줄일 수 없다」는 다른 말입니다.</b>',
    },
    {
        id: 'skew',
        name: '한 글자가 유난히 잦다',
        text: 'AAAAAAAAAABCDEFG',
        hint: 'A만 열 번이고 나머지는 한 번씩입니다. <b>허프만이 노리는 모양</b>입니다 — '
            + '다만 노린다는 것과 이긴다는 것은 다릅니다. 표에서 셋을 함께 보세요.',
    },
    {
        id: 'word',
        name: '같은 낱말이 반복된다',
        text: 'ABCABCABCABC',
        hint: '세 글자짜리 조각이 네 번 나옵니다. <b>키워드 인코딩이 노리는 모양</b>입니다.',
    },
    {
        id: 'same',
        name: '전부 같은 글자',
        text: 'AAAAAAAAAAAAAAAAAAAA',
        hint: '이어진 반복이 가장 긴 글입니다. 그런데 <b>허프만은 하나도 못 줄입니다</b> — '
            + '글자가 한 가지뿐이라 <b>짧은 코드를 몰아 줄 상대가 없기 때문</b>입니다.',
    },
];
 ...
 * 방법 셋.
 *
 * `run`은 스냅샷을 만들고, `decode`는 복원한다. **복원이 없으면 압축이 아니다** —
 * 검사가 회차마다 이것으로 원본과 대조한다.
 */
export const COMPRESS_METHODS = [
    {
        ...
        notes: [
            '「압축 전」 줄이 <b>원본</b>이고, 「압축 결과」 줄에 쌓이는 것이 <b>압축 결과</b>입니다.',
    ...
    },
    {
        ...
        idea: '자주 나오는 글자에 <b>짧은 코드</b>를, 드물게 나오는 글자에 긴 코드를 줍니다. '
            ...
            + '그래서 <b>횟수가 치우칠수록 많이 줄어듭니다.</b> '
            + '반대로 <b>횟수가 고르고 글자 가짓수가 2·4·8…이면 하나도 줄지 않습니다</b> — '
            + '그때는 같은 길이 코드가 이미 가장 짧기 때문입니다. '
            + '<b>가짓수가 3·5·6처럼 어정쩡하면 횟수가 고르더라도 조금 줄어듭니다.</b>',
        notes: [
            '가장 작은 둘을 묶어 올라가며 트리를 만듭니다. <b>자주 나오는 쪽이 왼쪽</b>입니다.',
            '루트 노드(Root)에서 단말 노드(Leaf)까지 걸어가며 <b>왼쪽 0, 오른쪽 1</b>을 모은 것이 그 글자의 코드입니다.',
            '<b>자주 나온 글자일수록 단말 노드가 위에 있어</b> 코드가 짧거나 같습니다. 횟수가 같은 글자끼리도 길이는 다를 수 있습니다.',
        ],
    },
    {
        id: 'keyword',
        ...
        idea: '떨어져 있어도 <b>반복되는 조각</b>을 기호 하나로 바꿉니다. '
            ...
            + '조각끼리 겹쳐 하나를 바꾸면 겹친 다른 조각이 깨져서 다 헤아려 보지 않고는 알 수 없기 때문입니다.',
        notes: [
            '「현재 글」 줄에서 <b>바뀐 자리가 기호로 바뀌어 갑니다.</b> 「압축 전」 줄은 그대로 둡니다 — '
            + '<b>무엇이 무엇으로 바뀌었는지 비교하라고</b> 둘을 함께 둡니다.',
            '바꾼 뒤 <b>남은 글자의 가짓수가 줄면 칸도 좁아집니다</b> — 줄이는 방식의 절반이 이것입니다. '
            + '반대로 <b>기호가 새 가짓수를 더해 칸이 넓어지면 오히려 늘어날 수도 있습니다.</b>',
...
/**
 * 함께 보내야 하는 것의 이름. **런 렝스에는 그런 것이 아예 없어 `null`이다.**
 *
 * 예전에는 셋 다 이름을 돌려주어 나란히 표에 「코드표 0비트」가 찍혔다.
 * 0이라는 값보다 **「그런 것이 없다」가 맞는 말**이고, 0으로 적어 두면 학생이
 * 「런 렝스에도 코드표가 있는데 마침 비었나 보다」로 읽는다.
 */
export const sideNameOf = (id) => ({keyword: '사전', huffman: '코드표'}[id] || null);
...
export const COMPRESS_COMMON_NOTES = [
    '크기는 <b>비트</b>로 셉니다. 압축 전은 <b>글자마다 같은 길이 코드를 줬을 때</b>입니다 — '
