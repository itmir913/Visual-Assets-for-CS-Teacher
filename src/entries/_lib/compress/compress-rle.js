 ...
 * 글자마다 「1번」을 덧붙이게 되어 **오히려 길어진다.** 그래서 프리셋에 반복이 없는
 * 글을 반드시 하나 둔다 → `compress-registry.js`
 *
 * **「두 배가 된다」고 적지 않는다.** 칸으로 세면 두 배가 맞지만 여기서는 비트로 센다 —
 * 횟수 자릿수가 글자 폭보다 좁으면 두 배가 안 되고, 넓으면 두 배를 넘는다.
 * `ABCDEFGHIJ`는 40비트에서 50비트로 **1.25배**다.
 *
