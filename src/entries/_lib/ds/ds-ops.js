 ...
 * 느리다」는 말은 외울 수 있지만, **뒤엣것이 하나씩 밀려나는 것을 보기 전에는**
 * 왜 느린지가 서지 않는다. 그래서 밀기 한 칸을 한 장으로 남긴다.
 *
 * **같은 이름의 연산이라도 담는 방식에 따라 하는 일이 다르다.** 그것이 이 파일이
...
/** 자리 `i`에 값을 넣는다. **뒤엣것을 뒤에서부터 하나씩 민다.** */
function arrayInsertAt(rec, i, v) {
    ...
    if (pushed > 0) {
        ...
        for (let j = rec.size - 1; j >= i; j--) {
            rec.say(`${j}번의 ${rec.peek(j).v}을(를) ${j + 1}번으로 밉니다.`);
            rec.shift(j, j + 1);
        }
    }
    rec.say(`빈 ${i}번 자리에 ${v}을(를) 씁니다.`);
    rec.write(i, dsItem(v));
    rec.setSize(rec.size + 1);
    rec.cursor('i', null);
    return pushed > 0
        ? `${v}을(를) ${i}번에 넣었습니다. **밀어낸 것이 ${pushed}개**입니다.`
        : `${v}을(를) ${i}번에 넣었습니다. **아무것도 밀지 않았습니다.**`;
}
...
function arrayRemoveAt(rec, i) {
    ...
    const gone = rec.at(i);
    rec.say(`${gone.v}을(를) 꺼내 그 자리를 비웁니다.`);
    rec.clear(i);
    ...
    if (pulled > 0) {
        ...
        for (let j = i + 1; j < rec.size; j++) {
            rec.say(`${j}번의 ${rec.peek(j).v}을(를) ${j - 1}번으로 당깁니다.`);
            rec.shift(j, j - 1);
        }
    }
    rec.setSize(rec.size - 1);
    rec.cursor('i', null);
    return pulled > 0
        ? `${gone.v}을(를) 뺐습니다. **당긴 것이 ${pulled}개**입니다.`
        : `${gone.v}을(를) 뺐습니다. **아무것도 당기지 않았습니다.**`;
}
...
function arrayReadAt(rec, i) {
    ...
    rec.clearFlag();
    rec.say(`${i}번 자리를 **바로 짚습니다.** 앞에서부터 세어 갈 것 없이`
        + ' 시작 주소에 자리 번호를 더하면 그 칸이 어디인지 곧바로 나옵니다.');
    const it = rec.at(i);
...
function arrayFind(rec, v) {
    ...
    for (let i = 0; i < rec.size; i++) {
        rec.say(`${i}번과 ${v}을(를) 견주어 봅니다.`);
        const it = rec.at(i);
        if (it.v === v) {
            rec.cursor('i', i);
            rec.say(`${i}번에서 ${v}을(를) 찾았습니다.`);
            rec.mark('found');
            rec.cursor('i', null);
    ...
    rec.say(`${v}은(는) 없습니다.`);
    rec.mark('missing');
    return `${v}은(는) 들어 있지 않습니다. **끝까지 ${rec.size}번 들여다보았습니다.**`;
}
...
/** 머리에서 `k`번째 마디까지 링크를 따라간다. 닿은 마디를 돌려준다. */
function walkTo(rec, k, name = 'p') {
    let nd = rec.nodeById(rec.head);
    if (!nd) return null;
    rec.say('머리 포인터가 가리키는 첫 마디에서 출발합니다.');
    rec.walk(nd.id, name);
    for (let step = 0; step < k; step++) {
        rec.say(`링크를 따라 다음 마디로 갑니다. (${step + 1}번째 걸음)`);
...
/** 자리 `i`의 **앞 마디**를 찾는다. 꼬리 포인터가 있으면 맨 뒤는 따라갈 것이 없다.
...
function nodeBefore(rec, i) {
    ...
    if (i >= rec.size && rec.state.hasTail) {
        rec.say('**꼬리 포인터**가 마지막 마디를 바로 가리킵니다. 따라갈 것이 없습니다.');
        return rec.walk(rec.tail, 'p');
    }
    if (i >= rec.size) {
        rec.say('꼬리 포인터가 없어 **마지막 마디가 어디인지 알 길이 없습니다.**'
            + ' 뒤에 넣는 것인데도 처음부터 끝까지 따라가야 합니다.');
    }
    return walkTo(rec, i - 1);
}

/** 마디를 자리 `i`에 끼워 넣는다. **밀리는 것이 하나도 없다.** */
function listInsertAt(rec, i, v) {
    ...
    rec.say(`값 ${v}을(를) 담은 **새 마디를 만듭니다.** 아직 어디에도 매달려 있지 않습니다.`);
    const nd = rec.newNode(v, i);

    // 새 마디가 뒤를 가리키게 먼저 건다. 앞을 먼저 끊으면 뒷줄을 놓쳐 잃어버린다.
    rec.say(after
        ? '**새 마디가 먼저 뒷마디를 가리키게 합니다.** 앞쪽을 먼저 끊으면 뒷줄을 통째로 잃습니다.'
        : '뒤에 아무것도 없으므로 새 마디의 링크는 비워 둡니다.');
    rec.link(nd.id, after ? after.id : null, 'next');

    if (before) {
        rec.say(`${i - 1}번 마디의 링크를 새 마디로 바꿉니다. **이 한 줄이 「끼워 넣기」의 전부입니다.**`);
        rec.link(before.id, nd.id, 'next');
    } else {
        rec.say('맨 앞에 넣는 것이므로 **머리 포인터**를 새 마디로 바꿉니다.');
        rec.link(null, nd.id, 'next');
    }

    if (s.doubly) {
        rec.say('되돌아가는 링크도 함께 겁니다. 이중 연결 리스트는 링크를 **양쪽 다** 고쳐야 합니다.');
        rec.link(nd.id, before ? before.id : null, 'prev');
        if (after) rec.link(after.id, nd.id, 'prev');
    }
    if (s.hasTail && !after) {
        rec.say('맨 뒤에 붙었으므로 **꼬리 포인터**도 새 마디로 옮깁니다.');
        rec.link(null, nd.id, 'prev');   // `prev` 방향은 꼬리 포인터를 뜻한다
    }

    rec.say('링크가 다 걸렸습니다. 새 마디가 줄에 들어왔습니다.');
    rec.settle(nd.id);
    rec.setSize(rec.size + 1);
    return `${v}을(를) ${i}번에 넣었습니다. **원소는 하나도 움직이지 않았습니다** — 고친 것은 링크뿐입니다.`;
}
...
function listRemoveAt(rec, i) {
    ...
    /* **되돌아가는 링크가 있으면 맨 뒤를 빼는 데 따라갈 것이 없다.** 이 갈래가 곧
       이중 연결 리스트를 두는 까닭이라, 단일 쪽과 나란히 보이게 말로도 밝힌다. */
    if (i === rec.size - 1 && s.doubly && rec.size > 0) {
        rec.say('**꼬리 포인터**로 마지막 마디에 바로 갑니다.');
        target = rec.walk(rec.tail, 'p');
        if (target.prev !== null) {
            rec.say('**되돌아가는 링크**가 있어 앞마디도 곧바로 알 수 있습니다.');
            before = rec.walk(target.prev, 'q');
        }
    } else {
        if (i === rec.size - 1 && rec.size > 1) {
            rec.say('마지막 마디를 떼려면 **그 앞마디의 링크**를 고쳐야 하는데,'
                + ' 되돌아가는 링크가 없어 앞마디를 알 수 없습니다. 처음부터 따라갑니다.');
        }
        before = i > 0 ? walkTo(rec, i - 1) : null;
        target = before ? rec.nodeById(before.next) : rec.nodeById(rec.head);
        if (target && before) {
            rec.say('빼려는 마디를 찾았습니다.');
            rec.walk(target.id, 'p');
        }
    }

    if (!target) {
        rec.flag(`${i}번 마디가 없습니다.`);
        rec.mark('empty');
        return '그 자리에는 마디가 없습니다.';
    }
    const after = rec.nodeById(target.next);

    rec.say(`${target.v}을(를) 담은 이 마디를 떼어 냅니다.`);
    rec.doom(target.id);

    if (before) {
        rec.say('앞마디의 링크를 **떼어 낼 마디를 건너뛰어** 그 뒤로 잇습니다.');
        rec.link(before.id, after ? after.id : null, 'next');
    } else {
        rec.say('맨 앞을 빼는 것이므로 **머리 포인터**를 다음 마디로 옮깁니다.');
        rec.link(null, after ? after.id : null, 'next');
    }
    if (s.doubly && after) {
        rec.say('되돌아가는 링크도 앞마디로 이어 줍니다.');
        rec.link(after.id, before ? before.id : null, 'prev');
    }
    if (s.hasTail && !after) {
        rec.say('마지막 마디였으므로 **꼬리 포인터**를 앞마디로 되돌립니다.');
    ...
    rec.setSize(rec.size - 1);
    return `${target.v}을(를) 뺐습니다. **밀거나 당긴 것은 하나도 없습니다.**`;
}

/** `k`번째 마디를 읽는다. **처음부터 링크를 따라가는 수밖에 없다.** */
function listReadAt(rec, k) {
    if (k >= rec.size) {
        rec.flag(`${k}번 마디가 없습니다.`);
        rec.mark('empty');
        return '그 자리에는 마디가 없습니다.';
    }
    rec.clearFlag();
    rec.say(`${k}번 마디를 보려 합니다. 마디들이 **여기저기 떨어져 있어** 자리 번호로`
        + ' 바로 짚을 수가 없습니다. 처음부터 링크를 따라갑니다.');
    const nd = walkTo(rec, k);
    rec.cursor('p', null);
    return `${k}번은 ${nd.v}입니다. **마디를 ${k + 1}개 지나왔습니다** —`
        + ' 뒤로 갈수록 더 걸립니다.';
}
...
function listFind(rec, v) {
    ...
    let seen = 0;
    while (nd) {
        rec.say(`이 마디의 값과 ${v}을(를) 견주어 봅니다.`);
        rec.walk(nd.id, 'p');
        seen++;
        if (nd.v === v) {
            rec.say(`${seen - 1}번째 마디에서 ${v}을(를) 찾았습니다.`);
            rec.mark('found');
            rec.cursor('p', null);
    ...
    rec.mark('missing');
    rec.cursor('p', null);
    return `${v}은(는) 들어 있지 않습니다. **끝까지 마디 ${seen}개를 지나왔습니다.**`;
}
...
export const dsArrayOps = {
    insertFront: {
        ...
        opening: (rec, {v}) => `${v}을(를) **맨 앞(0번)**에 넣으려 합니다.`,
        run: (rec, {v}) => arrayInsertAt(rec, 0, v),
    },
    insertBack: {
        id: 'insert-back', name: '뒤에 넣기', arg: 'value',
        opening: (rec, {v}) => `${v}을(를) **맨 뒤(${rec.size}번)**에 넣으려 합니다.`,
        run: (rec, {v}) => arrayInsertAt(rec, rec.size, v),
    },
    insertAt: {
        id: 'insert-at', name: 'k번째에 넣기', arg: 'valueIndex',
        opening: (rec, {v, i}) => `${v}을(를) **${i}번 자리**에 넣으려 합니다.`,
        run: (rec, {v, i}) => arrayInsertAt(rec, Math.min(i, rec.size), v),
    },
    removeFront: {
    ...
    removeAt: {
        ...
        run: (rec, {i}) => arrayRemoveAt(rec, Math.min(i, Math.max(0, rec.size - 1))),
    },
    readAt: {
    ...
    find: {
        ...
        opening: (rec, {v}) => `${v}이(가) 어디 있는지 찾으려 합니다.`,
        run: (rec, {v}) => arrayFind(rec, v),
    },
};

/** 마디로 담는 구조가 쓰는 연산 만들기. 이름은 배열 쪽과 **일부러 같게** 둔다 —
 *  같은 이름의 연산이 얼마나 다른 일을 하는지가 이 페이지의 요점이기 때문이다. */
export const dsListOps = {
    insertFront: {
        id: 'insert-front', name: '앞에 넣기', arg: 'value',
        opening: (rec, {v}) => `${v}을(를) **맨 앞**에 넣으려 합니다.`,
        run: (rec, {v}) => listInsertAt(rec, 0, v),
    },
    insertBack: {
        id: 'insert-back', name: '뒤에 넣기', arg: 'value',
        opening: (rec, {v}) => `${v}을(를) **맨 뒤**에 넣으려 합니다.`,
        run: (rec, {v}) => listInsertAt(rec, rec.size, v),
    },
    insertAt: {
        id: 'insert-at', name: 'k번째에 넣기', arg: 'valueIndex',
        opening: (rec, {v, i}) => `${v}을(를) **${i}번째**에 넣으려 합니다.`,
        run: (rec, {v, i}) => listInsertAt(rec, Math.min(i, rec.size), v),
    },
    removeFront: {
        id: 'remove-front', name: '앞에서 빼기', arg: null,
    ...
    removeAt: {
        ...
        run: (rec, {i}) => listRemoveAt(rec, Math.min(i, Math.max(0, rec.size - 1))),
    },
    readAt: {
        id: 'read-at', name: 'k번째 읽기', arg: 'index',
        opening: (rec, {i}) => `**${i}번째** 마디의 값을 보려 합니다.`,
 ...
 *  자리 번호를 받아 훑어 가는 「k번째 읽기」와 다른 점이다. */
function peekEnd(rec, i, where) {
    if (rec.size === 0) return emptyPeek(rec);
    rec.clearFlag();
    rec.say(`${where}이(가) 어느 칸인지 **늘 알고 있으므로** 바로 짚습니다.`);
    const it = rec.at(i);
    return `${where}은(는) ${it.v}입니다. **접근은 한 번**입니다.`;
}
    ...
    array: [
        {
            ...
            opening: (rec, {v}) => `${v}을(를) **꼭대기에 얹습니다.**`,
            run: (rec, {v}) => arrayInsertAt(rec, rec.size, v),
        },
        {
            id: 'pop', name: '빼기 (pop)', arg: null,
            opening: () => '**꼭대기**를 걷어 냅니다.',
            run: (rec) => arrayRemoveAt(rec, Math.max(0, rec.size - 1)),
        },
        {
            id: 'top', name: '꼭대기 보기 (peek)', arg: null,
            opening: () => '**꼭대기만 들여다봅니다.** 빼지는 않습니다.',
            run: (rec) => peekEnd(rec, rec.size - 1, '꼭대기'),
        },
    ],
    list: [
        {
        ...
        },
        {
            ...
            run: (rec) => {
                if (rec.size === 0) return emptyPeek(rec);
                rec.clearFlag();
                rec.say('**머리 포인터**가 꼭대기 마디를 바로 가리킵니다.');
                const nd = rec.walk(rec.head, 'p');
                return `꼭대기는 ${nd.v}입니다. **마디 하나만 지났습니다.**`;
            },
        },
    ],
};
...
export const dsQueueOps = {
    array: [
        ...
        },
        {
            ...
            opening: () => '**맨 앞만 들여다봅니다.** 내보내지는 않습니다.',
    ...
    list: [
        ...
        },
        {
            ...
            run: (rec) => {
                if (rec.size === 0) return emptyPeek(rec);
                rec.clearFlag();
                rec.say('**머리 포인터**가 줄 맨 앞을 바로 가리킵니다.');
                const nd = rec.walk(rec.head, 'p');
                return `맨 앞은 ${nd.v}입니다. **마디 하나만 지났습니다.**`;
            },
        },
    ],
};
    ...
    array: [
        {
            ...
            opening: (rec, {v}) => `${v}을(를) **앞쪽 끝**에 넣습니다.`,
            run: (rec, {v}) => arrayInsertAt(rec, 0, v),
        },
        {
            id: 'push-back', name: '뒤에 넣기', arg: 'value',
            opening: (rec, {v}) => `${v}을(를) **뒤쪽 끝**에 넣습니다.`,
            run: (rec, {v}) => arrayInsertAt(rec, rec.size, v),
        },
        {
...
/* ---------------------------------------------------------------
   ...
   재어 본다** — 개수를 키워 가며 작업량이 어떻게 자라는지를 보고 여기 적힌 것과 맞춘다.
   카드에 적힌 말과 화면에서 벌어지는 일이 어긋나는 것이 가장 나쁜 결함이라,
   말을 사람이 적는 이상 기계가 붙들어야 한다.

   **상태를 받는 함수인 것에 뜻이 있다.** 같은 「뒤에 넣기」라도 꼬리 포인터가 있으면
...
export const dsRingOps = [
    {
        id: 'enqueue', name: '넣기 (enqueue)', arg: 'value',
        opening: (rec, {v}) => `${v}을(를) **rear 자리**에 넣으려 합니다.`,
        run: (rec, {v}) => {
            const s = rec.state;
            if (rec.size >= rec.cap) {
            ...
            rec.clearFlag();
            const at = s.rear;
            rec.say(`rear가 가리키는 ${at}번 칸에 ${v}을(를) 씁니다.`);
            ...
            rec.mark('ring');
            return `${v}을(를) 넣었습니다. **아무것도 밀지 않았습니다.**`;
        },
    },
    {
        id: 'dequeue', name: '빼기 (dequeue)', arg: null,
        ...
        run: (rec) => {
            ...
            rec.say(`${gone.v}을(를) 꺼내고 그 칸을 비웁니다.`);
            rec.clear(at);
            rec.setSize(rec.size - 1);
            const next = (at + 1) % rec.cap;
            s.front = next;
            ...
            rec.mark('ring');
            return `${gone.v}을(를) 뺐습니다. **당긴 것이 하나도 없습니다** —`
                + ' 원소가 아니라 front가 움직였습니다.';
        },
    },
    {
        id: 'front', name: '맨 앞 보기 (peek)', arg: null,
        opening: () => '**front 자리**만 들여다봅니다. 빼지는 않습니다.',
        run: (rec) => {
            if (rec.size === 0) {
                rec.flag('비어 있습니다.');
                rec.mark('empty');
