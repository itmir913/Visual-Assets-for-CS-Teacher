...
const arrayStruct = {
    ...
    idea: '칸을 **빈틈없이 붙여 놓고** 인덱스로 접근합니다. 칸의 크기가 모두 같아서 '
        + '「시작 주소 + 인덱스 × 칸 크기」로 몇 번째든 **한 번에** 찾아갑니다. '
    ...
    watch: '「앞에 삽입」과 「뒤에 삽입」을 번갈아 눌러 보세요. 넣는 값은 하나로 같은데 '
        ...
        + '「k번째 읽기」는 k를 아무리 키워도 접근이 1입니다 — 이것이 배열의 강점입니다.',
    costRows: [
        ['앞에 삽입 · 삭제', 'O(n)', '뒤에 있는 원소를 전부 민다'],
        ['뒤에 삽입 · 삭제', 'O(1)', '밀 것이 없다'],
        ['k번째 읽기', 'O(1)', '인덱스로 바로 접근한다'],
        ['값 찾기', 'O(n)', '처음부터 차례로 비교한다'],
    ],
    facts: [
        {on: true, text: '인덱스로 바로 접근하기', hint: '몇 번째를 묻든 한 번에 찾아갑니다'},
...
const singlyStruct = {
    ...
    costRows: [
        ...
        ['k번째 읽기', 'O(n)', 'head부터 링크를 따라간다'],
        ['값 찾기', 'O(n)', '처음부터 차례로 비교한다'],
    ],
    facts: [
        {on: true, text: '넣고 뺄 때 밀지 않음', hint: '고치는 것은 링크뿐이라 원소는 제자리에 있습니다'},
        {on: false, text: '인덱스로 접근하지 못함', hint: 'k번째를 보려면 처음부터 따라가야 합니다'},
    ],
};
...
export const DS_COMPARE = {
    ...
    watch: '「앞에 삽입」에서는 배열이 밀리느라 오래 걸리고, 「k번째 읽기」에서는 '
    ...
    costRows: [
        ...
        ['k번째 읽기', '배열 O(1) · 리스트 O(n)', '접근하느냐 따라가느냐'],
