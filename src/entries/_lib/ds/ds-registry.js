...
const singlyStruct = {
    ...
    idea: '값과 **다음 마디의 자리**를 함께 담은 마디를 링크로 잇습니다. '
...
const doublyStruct = {
    ...
    watch: '「뒤에서 빼기」를 단일 연결 리스트와 번갈아 해 보세요. 단일 쪽은 **앞마디를 '
        + '알 수 없어** 처음부터 다시 따라가야 하지만, 이쪽은 되돌아가는 링크로 곧장 갑니다. '
...
function adtStruct(spec) {
    return {
        ...
        impls: [
            ...
            {id: 'list', name: '연결 리스트로 담기', view: 'list', ops: spec.ops.list},
        ],
...
const ringStruct = {
    ...
    idea: '칸을 **동그랗게 이어 붙였다고 치고** 씁니다. 실제로는 그냥 배열이고, '
        ...
        + '**front와 rear라는 자리 번호 둘**이 어디부터 어디까지가 줄인지를 기억합니다.',
...
const dequeStruct = adtStruct({
    ...
    watch: '배열로 담으면 **앞쪽 끝이 여전히 비쌉니다** — 앞에 넣으려면 밀어야 하니까요. '
        + '연결 리스트(이중)로 담으면 양쪽이 모두 싸집니다. '
