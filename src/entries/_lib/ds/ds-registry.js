...
const doublyStruct = {
    ...
    watch: '「뒤에서 빼기」를 단일 연결 리스트와 번갈아 해 보세요. 단일 쪽은 **앞 노드를 '
...
const stackStruct = adtStruct({
    ...
    watch: '저장 구조를 바꿔 가며 눌러 보세요. **배열로 담아도 미는 일이 없습니다** — '
        + '넣고 빼는 자리가 늘 끝이기 때문입니다. 「앞에 넣기」의 비용이 큰 배열이 스택에서는 '
...
const queueStruct = adtStruct({
    ...
    costRows: [
        ...
        ['넣기 (enqueue) · 연결 리스트', 'O(1)', 'tail 포인터로 곧장 간다'],
        ['빼기 (dequeue) · 배열', 'O(n)', '뒤에 있는 원소를 모두 당긴다'],
        ['빼기 (dequeue) · 연결 리스트', 'O(1)', 'head 포인터만 옮긴다'],
    ],
    facts: [
        {on: true, text: '선입선출 (FIFO)', hint: '먼저 넣은 것이 먼저 나옵니다'},
        {on: false, text: '이렇게 담으면 빼기의 비용이 크다', hint: '배열에 앞에서부터 담으면 맨 앞이 빠진 자리를 메우려고 모두 당깁니다. 다음 탭의 원형 큐가 그것을 없앱니다'},
    ],
});
...
const dequeStruct = adtStruct({
    ...
    watch: '**이 시뮬레이터의 배열 구현은 앞쪽 끝의 비용이 큽니다** — 앞에 넣으려면 밀어야 하니까요. '
        ...
        + '여기서 가장 뚜렷하게 보입니다.',
    costRows: [
        ['앞에 넣기 · 빼기 · 배열', 'O(n)', '뒤에 있는 원소를 민다'],
        ['앞에 넣기 · 빼기 · 이중 연결 리스트', 'O(1)', 'head 포인터만 고친다'],
