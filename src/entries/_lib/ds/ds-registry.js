...
export const DS_GROUPS = [
    {
        ...
        blurb: '값을 **어디에 어떻게 늘어놓는가**에 대한 답입니다. '
            ...
            + '링크로 잇습니다. 여기서 갈린 성질이 아래 「추상 자료형」의 비용을 정합니다.',
    },
    {
        id: 'adt',
        name: '추상 자료형',
...
const queueStruct = adtStruct({
    ...
    facts: [
        ...
        {on: false, text: '이렇게 담으면 삭제의 비용이 크다', hint: '배열에 앞에서부터 담으면 맨 앞이 빠진 자리를 메우려고 모두 당깁니다. 다음 탭의 원형 큐가 그것을 없앱니다'},
    ],
});
...
const ringStruct = {
    ...
    watch: '삽입과 삭제를 여러 번 눌러 **front와 rear가 끝을 지나 0으로 돌아오는 것**을 보세요. '
        + '앞 탭의 큐와 비교하면 요점이 뚜렷합니다 — **당기는 일이 아예 없습니다.** '
...
const dequeStruct = adtStruct({
    ...
    watch: '**이 시뮬레이터의 배열 구현은 앞쪽 끝의 비용이 큽니다** — 앞에 넣으려면 밀어야 하니까요. '
        ...
        + '**다만 배열이라서 비용이 큰 것은 아닙니다** — 앞 탭의 원형 큐처럼 인덱스를 돌려 쓰면 '
    ...
    costRows: [
        ...
        ['앞에 삽입 · 삭제 · 원형으로 쓴 배열', 'O(1)', '앞 탭의 수를 쓰면 밀지 않아도 된다'],
...
export const DS_COMPARE = {
    ...
    watch: '「앞에 삽입」에서는 배열이 밀리느라 오래 걸리고, 「k번째 읽기」에서는 '
        ...
        + '아래 표는 개수를 키워 가며 측정한 것입니다 — 차이가 개수에 따라 어떻게 벌어지는지 보세요.',
    costRows: [
