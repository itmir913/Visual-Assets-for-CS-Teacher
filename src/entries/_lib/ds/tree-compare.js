 ...
 * 편향되는 것을 보기 전에는** 그 말이 무슨 뜻인지 서지 않는다. 그리고 그 최악은
...
export function buildTreeCompare(op, states, arg) {
    ...
    for (let t = 0; t <= maxWork; t++) {
        ...
        frames.push({
            ...
            say: finished === 0
                ? '두 트리에 **같은 값을 같은 순서로** 넣습니다. 작업량(비교 + 이동 + 링크)을'
...
function closingLine(runs, lanes) {
    ...
    if (hb === ha) {
        return '둘 다 끝났습니다. **이번에는 높이가 같습니다** — '
            + '넣는 순서가 좋으면 이진 탐색 트리도 잘 자랍니다.';
    }
    /* **두 수 쌍의 순서를 반드시 맞춘다.** 앞에서 「이진 탐색 트리 대 AVL」로 적어 놓고
       뒤에서 「AVL 대 이진 탐색 트리」로 적었더니, 학생이 앞 차례로 뒤를 읽어
       **작업량을 정반대로** 가져갔다. 그리고 어느 쪽이 더 들었는지는 회차마다 다르므로
       「AVL이 더 든다」로 못박지 않는다 — 실제로 측정한 값으로 말한다. */
    const wb = runs[0].total;
    const wa = runs[1].total;
    const cost = wa === wb
        ? `이번 회차는 **작업량이 ${withJosa(wb, '으로')} 같습니다.**`
        : (wa > wb
    ...
    return `둘 다 끝났습니다. **높이가 ${hb} 대 ${ha}입니다**(이진 탐색 트리 대 AVL). `
        + `AVL 트리는 회전으로 높이를 ${withJosa(ha, '으로')} 눌러 두었습니다. ${cost}`;
}
...
/** 늘 같은 순서로 섞는다. **씨앗을 못박는다** — 볼 때마다 표가 달라지면 비교할 수가 없다. */
function shuffled(n) {
