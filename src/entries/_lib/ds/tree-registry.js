...
export const TREE_GROUPS = [
    {
        ...
        blurb: '**「이 값이 어디 있나」**에 답하는 트리입니다. 왼쪽은 작고 오른쪽은 크다는 '
            + '규칙 하나로, 한 번 견줄 때마다 **볼 곳이 반으로 줄어듭니다.** '
...
const bstStruct = {
    ...
    idea: '마디마다 **왼쪽 가지에는 자기보다 작은 값만, 오른쪽 가지에는 큰 값만** 둡니다. '
        + '그래서 찾을 때 한 번 견줄 때마다 **반대쪽 가지를 통째로 버릴 수 있습니다.** '
    ...
    costRows: [
        ['찾기 · 넣기 · 빼기', 'O(높이)', '한 층 내려갈 때마다 한 번 견준다'],
...
const avlStruct = {
    ...
    watch: '「비우기」를 누르고 10 · 20 · 30을 차례로 넣어 보세요. 30을 넣는 순간 **돌아갑니다.** '
        + '같은 값을 이진 탐색 트리에 넣은 것과 대 보면 높이가 다릅니다. '
        + '**회전이 공짜가 아니라는 것도 보세요** — 링크를 세 번 고쳐 씁니다. '
        + '그리고 돌린 뒤 **중위 순회**를 해 보세요. 자리는 바뀌었는데 **차례는 그대로**입니다.',
    costRows: [
        ['찾기 · 넣기 · 빼기', 'O(log n)', '높이가 늘 log n 언저리로 눌려 있다'],
        ['넣을 때의 회전', '많아야 1번', '한 번 펴면 높이가 되돌아온다'],
        ['뺄 때의 회전', 'O(log n)번', '뿌리까지 올라가며 봐야 한다'],
        ['순회', 'O(n)', '모든 마디를 한 번씩 지난다'],
    ],
    facts: [
...
const heapStruct = {
    ...
    watch: '**트리와 배열을 함께 보세요.** 같은 것을 두 가지로 그린 것입니다. '
        ...
        + '`2 × 자리 + 1`이 왼쪽 자식입니다. '
        + '**「값 찾기」를 눌러 보세요** — 이진 탐색 트리와 달리 하나씩 다 봅니다. '
        + '힙이 잘하는 것은 「가장 큰 것 꺼내기」이지 「찾기」가 아닙니다.',
    costRows: [
        ['넣기', 'O(log n)', '부모와 견주며 올라간다'],
        ['가장 큰 값 꺼내기', 'O(log n)', '뿌리를 꺼내고 자식과 견주며 내려간다'],
