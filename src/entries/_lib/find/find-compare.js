/* 나란히 비교 — **같은 자료에서 같은 값을 세 가지 방법으로 한꺼번에 찾는다.**
 *
...
/** 나란히 비교가 쓰는 해시 표의 칸 수. **값 수에 맞춰 늘린다** — 까닭은 `measureFindWork`에. */
...
export function buildFindRace(states, v) {
     ...
     *
     * **「작업량이 셋 다 0인가」로 물으면 안 된다.** 순차·이진은 빈 자료에서 곧장
     * 물러나 0이지만 해시는 **비어 있어도 계산 한 번과 칸 한 번을 쓴다** — 그것이
     * 해시의 성질이다. 그래서 그 가지는 영영 안 타고, 빈 자료에서 「순차 탐색이 0으로
     * 이겼다」는 이 페이지가 가르치려는 것의 **정반대**가 화면에 남았다.
     * 물어야 할 것은 든 값이 아니라 **담긴 것이 있는가**다. */
    if (states.seq.size === 0) {
        return {
            frames: [{
                lanes: runs.map((r) => ({
                    kind: r.kind,
...
/** 아래 표에서 개수를 키워 가며 측정하는 자리. */
export const FIND_MEASURE_SIZES = [8, 16, 32, 64, 128];

/** 한 크기에서 몇 개의 값을 찾아 평균을 낼지. **전부 측정하면 느리고, 하나만 측정하면 고른 티가 난다.** */
const SAMPLES = 8;

/**
 * **장을 남기지 않고 개수를 키워 가며 작업량만 측정한다.**
 *
 * **고르게 뽑은 여덟 값의 평균을 쓴다.** 한 값만 측정하면 어느 것을 고르느냐로 답이 달라진다 —
 * 순차 탐색은 첫 값이면 1이고 끝 값이면 n이다. 고른 값 하나로 표를 만들면 그것은 측정이
 * 아니라 **고르는 것**이 된다.
 *
 * **해시 표는 값 수에 맞춰 키운다.** 칸을 10으로 고정해 두고 값을 128개 넣으면 한 칸에
 * 열세 개씩 연결되어 해시도 결국 O(n)이 된다 — 그러면 표가 「해시는 그대로다」를 부정하게 된다.
