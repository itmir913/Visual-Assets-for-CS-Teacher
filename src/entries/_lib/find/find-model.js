 ...
 * 「연산 한 번이 한 회차」인 것도 같다. 앞 회차가 끝난 상태를 물려받아 시작한다.
 *
 * **다른 것은 무엇을 세느냐다.** 이 페이지가 답하는 질문은 하나다 —
 * 「같은 자료에서 같은 값을 찾는데, 왜 어떤 방법은 자료가 늘어날수록 느려지고 어떤 방법은
 * 그대로인가.」 그래서 세는 횟수를 셋으로 갈랐다.
 *
 *   `비교`  값을 비교한다                     순차는 n번, 이진은 log n번, 해시는 몇 번
 *   `접근`  칸이나 노드를 하나 들여다본다      대개 비교와 함께 가지만 체이닝에서 갈린다
...
export function createFindRecorder(state) {
    ...
    let hit = [];          // 값이 맞아떨어진 칸
    let hitPos = null;     // 사슬에서 맞아떨어진 노드의 자리(체이닝에서만)
    let ruled = [];        // 방금 «버린» 자리 — 이진 탐색이 반씩 지워 가는 것을 보인다
    let banner = null;     // 회차 전체에 붙는 말
    ...
    const rec = {
        ...
        /**
         * 칸 하나를 들여다보고 **값을 비교한다.** 찾기에서 실제로 일어나는 일은 이것이다.
        ...
        /** 칸 하나를 들여다본다. 값을 비교하지는 않는다 — 비었는지 보는 것도 «접근»이다. */
        visit(i) {
            counts.access += 1;
            focus = [i];
            snap({kind: 'visit', i});
        ...
        erase(i, chainPos = null, tomb = false) {
            ...
            s.size -= 1;
            focus = [i];
            /* **지운 자리의 「찾던 값」 표시를 끈다.** `hit`은 회차가 끝날 때까지 남기는
               것이 규칙인데, 빼기에서는 그 규칙이 거꾸로 문다 — 사슬에서 노드를 빼면
