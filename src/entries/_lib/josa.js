 ...
 * 알파벳은 **낱글자로 읽을 때만** 판정한다(한 글자이거나 전부 대문자).
 * L 엘 · M 엠 · N 엔만 받침이 있고, 마지막 글자가 조사를 정한다.
...
function tailOf(text) {
    ...
    /* 알파벳은 읽는 소리가 정해져 있어 판정할 수 있다 — 「L이」와 「A가」다.
       **낱글자로 읽는 것만 받는다** — 한 글자이거나, 전부 대문자여서 「엠엔」처럼
       한 자씩 읽는 것이다. 그때는 마지막 글자가 조사를 정한다.
       **「data」 같은 낱말은 여기 들어오지 않는다.** 「데이터」로 읽을지 「다타」로 읽을지
       기계가 알 수 없기 때문이다.
       그래프 노드 이름과 압축 시뮬레이터의 조각이 이 자리를 쓴다. */
    if (/^[A-Za-z]$/.test(s) || /^[A-Z]+$/.test(s)) {
