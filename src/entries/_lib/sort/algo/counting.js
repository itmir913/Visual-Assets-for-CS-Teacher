/* 계수 정렬.
 *
 * **이 종목이 있어야 「정렬은 O(n log n)이 한계」라는 말이 조건부라는 것이 드러난다.**
...
export const countingSortAlgo = {
    ...
    run(rec) {
        ...
        for (const v of keys) {
            ...
            rec.stripFocus(v);
            if (!have) {
                rec.say(`값 ${v}는 한 번도 나오지 않았습니다. 건너뜁니다.`);
                rec.mark('skip');
                continue;
            }
            rec.say(`값 ${v}가 ${have}개 있습니다. 넣은 차례 그대로 꺼내 씁니다.`);
