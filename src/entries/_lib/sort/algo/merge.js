/* 병합 정렬.
 *
 * **보조 칸을 숨기지 않는다.** C 교과서에서 흔히 쓰는 꼴 그대로, 합칠 두 조각을
 * 왼쪽·오른쪽 칸으로 따로 떠 온 뒤 세 손가락으로 되돌려 쓴다. 그래야
 * 「제자리 정렬이 아니다」가 글이 아니라 **화면에 실제로 나타났다 사라지는 칸**이 된다.
 *
 * 구현이 이것 하나뿐인 것은 아니다 — 임시 배열을 하나만 쓰는 판, 두 버퍼를 번갈아
...
export const mergeSortAlgo = {
    ...
    view: 'array',
    motion: 'write',
    idea: '구간을 반씩 쪼개 더 쪼갤 수 없을 때까지 내려간 다음, '
        + '정렬된 두 조각을 앞에서부터 하나씩 견주며 합쳐 올라옵니다.',
    ...
    watch: '자료가 어떻게 생겼든 걸리는 시간이 거의 같습니다 — 무작위든 역순이든 '
        ...
        + '값이 같으면 왼쪽 조각을 먼저 가져오기 때문에 앞뒤 차례가 지켜집니다(안정 정렬).',
    ...
    run(rec) {
        ...
        const msort = (lo, hi, depth) => {
            ...
            rec.say('정렬된 두 조각을 따로 떠 옵니다. 이 칸이 병합 정렬이 더 쓰는 자리입니다.');
            // 이름을 짧게 둔다 — 조각이 한 칸짜리일 때 이름표가 칸보다 길어진다.
            rec.auxOpen([{label: '왼쪽', base: lo}, {label: '오른쪽', base: mid + 1}]);
            rec.auxFill(0, lo, mid);
            rec.auxFill(1, mid + 1, hi);

            // **떠 온 뒤 원래 구간을 비운다.** 여기에 처음부터 다시 채워 넣을 것이므로,
            // 옛 값이 남아 있으면 「어디까지 채웠는지」가 화면에서 구별되지 않는다.
            rec.vacate(lo, hi);
            rec.say('두 조각의 맨 앞끼리 비교해 작은 쪽을 빈 구간에 차례로 씁니다.');
            ...
            while (x < rec.auxLen(0)) {
                rec.cursor('쓸 자리', k);
                rec.say('오른쪽 조각이 먼저 비었습니다. 왼쪽에 남은 것은 그대로 옮깁니다.');
                rec.auxWriteBack(0, x++, k++);
            }
            while (y < rec.auxLen(1)) {
                rec.cursor('쓸 자리', k);
                rec.say('왼쪽 조각이 먼저 비었습니다. 오른쪽에 남은 것은 그대로 옮깁니다.');
