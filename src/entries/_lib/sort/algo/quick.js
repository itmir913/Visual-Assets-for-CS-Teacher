...
export const quickSortAlgo = {
    ...
    view: 'array',
    motion: 'swap',
    idea: '**분할 정복**을 병합 정렬과 반대 순서로 씁니다. 구간에서 기준값(피벗) 하나를 고르고 '
    ...
    run(rec) {
        const qsort = (lo, hi, depth) => {
            ...
            rec.pivotAt(hi);
            rec.say(`인덱스 ${lo}~${hi} 구간 — 맨 끝 값을 피벗으로 삼고, 이보다 작은 것을 왼쪽으로 몰아냅니다.`);
            rec.mark('pivot');
            ...
            rec.cursor('경계', null);
            rec.fix(wall);
            rec.say(`피벗이 인덱스 ${wall}에 확정되었습니다. 왼쪽은 모두 작고 오른쪽은 모두 크거나 같습니다.`);
            rec.mark('fix');
