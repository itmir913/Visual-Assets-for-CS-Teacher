 ...
 * **한 번에 한 자릿수씩** 보아 칸을 언제나 열 개로 묶는다. 대신 자릿수만큼 회차를 돈다.
 *
 * **꺼낼 때 넣은 순서를 지키는 것이 이 알고리즘의 전부다.** 그 순서가 흐트러지면
...
export const radixSortAlgo = {
    ...
    stable: true,
    inPlace: false,
    watch: '칸에서 꺼낼 때 **넣은 순서를 그대로** 지키는지 보세요. '
        + '그 순서가 흐트러지면 앞 자릿수에서 해 놓은 정렬이 무너져 결과가 틀립니다 — '
        + '기수 정렬에서 안정성은 있으면 좋은 성질이 아니라 없으면 안 되는 조건입니다. '
        + 'd는 자릿수입니다. 계수 정렬과 달리 칸은 늘 열 개뿐이지만, 자릿수만큼 회차를 돕니다.',
