 ...
 *
 * 탭바도, 알고리즘 비교 목록도, 검사도 전부 여기서 읽는다 —
...
export const SORT_GROUPS = [
    ...
    },
    {
        id: 'race',
        name: '알고리즘 비교',
        blurb: '같은 자료를 열한 가지 알고리즘에 한꺼번에 물리고 **작업량을 똑같이 나눠 줍니다** — '
            + '화면에 뜨는 세 숫자(비교 · 옮김 · 배열 접근)를 더한 값입니다. '
            + '그래서 먼저 끝난 쪽이 실제로 일을 덜 한 것입니다. '
            + '작업량 곡선은 원소 수를 키워 가며 측정한 것으로, 한 단계씩 넘겨서는 볼 수 없는 그림입니다.',
    },
];
...
/* **알고리즘 비교는 알고리즘이 아니다.** 그래도 탭에는 나와야 하므로 알고리즘처럼 생긴 항목을
   하나 둔다. `SORT_ALGOS`에는 넣지 않는다 — 넣으면 알고리즘 비교가 자기 자신과 비교된다. */
export const SORT_RACE = {
    ...
    group: 'race',
    view: 'race',
    idea: '고른 알고리즘에 **같은 자료를 똑같이** 물리고 작업량을 한 칸씩 나란히 넘깁니다. '
        + '한 알고리즘만 볼 때는 숫자로만 남던 차이가, 나란히 놓으면 끝나는 차례로 드러납니다.',
    ...
    watch: '자료를 「거의 정렬됨」으로 바꿔 보세요. 삽입 정렬이 분할 정복보다 먼저 끝납니다. '
        ...
        + '작업량 곡선은 가로(원소 수)와 세로(작업량)가 모두 로그 눈금이라, 선의 「기울기」가 복잡도의 차수를 나타냅니다.',
};

/** 탭에 나오는 것 전부 — 진짜 알고리즘 열에 알고리즘 비교를 더한 것. */
