...
/** 이보다 많으면 트리를 그리지 않는다. 5층이 넘으면 마디가 글자보다 좁아져 아무것도 안 보인다. */
...
/* **SVG 글자는 그리는 폭에 비례해 줄어든다.** viewBox 720짜리를 405px에 그리면
   ...
   둘 다 한다. 1만 하면 넓은 화면에서 글자가 커지고, 2만 하면 마디 동그라미가
   글자보다 작아진다. */
...
export function createSortHeapView(host) {
    ...
    function buildTree() {
        ...
        /* 층수에 맞춰 그림 폭을 정한다. 마디 하나에 90은 줘야 두 자리 숫자가 들어간다.
        ...
        svg = svgEl('svg', {
            ...
            /* 폭 천장·바닥·가운데 정렬을 함께 준다. 천장이 없으면 넓은 화면에서 도형만
               과하게 커지고, 바닥이 없으면 좁은 화면에서 마디가 글자보다 작아지며,
    ...
    function paintTree(frame) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            /* **힙에서 빠져나간 마디는 흐리게 둔다.** 지우면 트리 모양이 무너져
