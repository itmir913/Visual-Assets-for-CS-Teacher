// 「simulator/cs/compress.html」이 쓰는 것을 담는다.
//
...
import {mountCompressSimulator} from '../../_lib/compress/compress-ui.js';

// 모듈 진입점은 파싱이 끝난 뒤 도므로 이 시점에 뼈대는 이미 DOM에 있다.
// 그래도 `readyState`를 본다 — 빌드가 이 파일을 head의 `defer` 스크립트로 바꾸기 때문이다.
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', mountCompressSimulator);
} else {
    mountCompressSimulator();
}
