// 「simulator/cs/sort.html」이 쓰는 것을 담는다.
//
// **이 페이지는 인라인 스크립트가 없다.** HTML은 뼈대만 두고 화면을 여기서 채운다 —
// 정렬이 여러 알고리즘이라 한 장에 다 적으면 파일이 통째로 읽히지 않는 크기가 된다.
//
...
import {mountSortSimulator} from '../../_lib/sort/sort-ui.js';

// 모듈 진입점은 파싱이 끝난 뒤 도므로 이 시점에 뼈대는 이미 DOM에 있다.
// 그래도 `readyState`를 본다 — 빌드가 이 파일을 head의 `defer` 스크립트로 바꾸기 때문이다.
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', mountSortSimulator);
} else {
    mountSortSimulator();
}
