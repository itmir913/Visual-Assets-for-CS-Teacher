    ...
    for (const start of [[], [9], [...DS_START], Array.from({length: DS_CAP}, (_, k) => k + 1)]) {
        for (const op of struct.ops) {
            for (const arg of ARGS) {
                ...
                /* **저장 구조가 세는 값을 정한다.** 연결 리스트가 원소를 옮기거나
                ...
                /* **끝 장의 설명이 센 수가 실제로 민 수와 같은가.** 학생이 외우는 것은 이 문장이다.
                   이 줄이 없던 동안 「밀어낸 것」을 하나 더 세는 돌연변이가 살아남았다(2026-09-25). */
                const plainSay = last.say.replace(/\*\*/g, '');
                // 「민」 것은 `shift` 장이다. `counts.move` 는 제자리에 쓰는 것까지 세므로 쓰지 않는다.
                const shifted = out.frames.filter((f) => f.act.kind === 'shift').length;
                const said = plainSay.match(/밀어낸 것이 (\d+)개/);
                if (said && Number(said[1]) !== shifted) {
                    bad(`${struct.name} · ${op.name} — 「밀어낸 것이 ${said[1]}개」라는데 실제로 ${shifted}번 밀었다`);
                }
                if (plainSay.includes('아무것도 밀지 않았습니다') && shifted !== 0) {
                    bad(`${struct.name} · ${op.name} — 「아무것도 밀지 않았다」는데 ${shifted}번 밀었다`);
                }
            }
        }
    }
...
/* ================================================================
   2. 추상 자료형이 정해진 구조 — **규칙 그대로 흉내 낸 모형과 맞춘다**

   스택이 후입선출인지, 큐가 선입선출인지는 «담긴 차례»로만 확인할 수 있다.
   그래서 평범한 배열로 같은 규칙을 흉내 내고, 연산을 무작위로 섞어 물린 뒤
   양쪽에 남은 것을 비교한다. 원형 큐는 **자리가 끝을 지나 돌아간 뒤**가 요점이므로
...
function mirrorOf(structId) {
    ...
    return {
        ...
        /** 화면이 내놓는 차례로 맞춰 준다. 리스트로 담은 스택은 맨 위가 앞에 온다. */
        values(kind) {
...
for (const spec of ADT) {
    ...
    for (const implId of spec.impls) {
        ...
        for (let step = 0; step < 120; step++) {
            ...
            /* **꽉 찼거나 비어 있어 아무 일도 못 한 회차은 모형에도 물리지 않는다.**
...
console.log(`추상 자료형이 정해진 구조 넷 — ${adtChecks}걸음을 흉내 모형과 걸음마다 맞췄다`);
...
for (const struct of DS_STRUCTS) {
    for (const implId of (struct.impls ? struct.impls.map((i) => i.id) : [null])) {
        ...
        for (const op of plan.ops) {
            if (!op.cost) { bad(`${struct.name} · ${op.name} — 비용이 적혀 있지 않다`); continue; }
...
console.log(`비용 ${costChecks}가지 — 개수를 8에서 64로 키워 가며 적어 둔 것과 맞췄다`);
...
/* ================================================================
   ...
   화면이 그렇게 말한다. 그 말이 참이려면 끝나는 순서가 작업량의 순서와 같아야 한다.
 ...
 * 꽉 차면 더 못 넣는데 리스트는 계속 들어가, **두 줄이 갈라진 채로 비교하고 있었다.**
...
console.log('페이지를 띄워 구조·저장 구조·연산을 모두 눌러 보았다');
for (const r of pageRows) console.log('  ' + r);

/* **연산을 눌러도 그림 상자의 높이가 흔들리면 안 된다.**
   단계를 넘길 때 상자가 늘었다 줄었다 하면 그 아래 버튼이 아래위로 움직이고,
   그러면 같은 자리를 거듭해 누를 수가 없다. */
/* **높이를 어디에 적었든 찾아낸다.**
 *
 * 예전에는 `el.style.height`만 보았다. 그런데 노드 그림은 SVG 크기를 `style` **속성
...
for (const group of [...page.el('group-tabs').children]) {
    ...
    for (const chip of [...page.el('struct-tabs').children]) {
        const structName = chip.textContent;
        chip.click();
        /* **저장 구조를 첫째 것으로 되돌린다.** 앞 절이 마지막으로 누른 것이 그대로
           남아 있어, 스택·큐·덱까지 노드 그림으로 재고 있었다. */
        ...
        if (moved) {
            bad(`${structName} — 단계를 넘기는 동안 상자 높이가 바뀌었다(${moved}). 버튼이 움직인다`);
        }
    }
}
...
test('ds', () => { expect(fail, '위 ✗ 줄을 볼 것').toBe(0); });
