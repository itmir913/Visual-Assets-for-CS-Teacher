...
//   5. **화면에 그린 굵은 선이 고른 이웃과 하나씩 대응하는가.** 여기가 어긋나면 셈은 맞는데
//      **학생이 보는 그림만 틀린다.**
...
//   7. **무작위 자료가 판 안에 들어오고, 판 밖으로 나간 점을 가장자리 선 위로 끌어와 쌓지 않는가.**
//   8. **손으로 짠 동점.** 반이 셋이면 K 가 홀수여도 동점이 난다. 동점에 끼지 못한 반의 이웃이
//      가장 가까워도 따르지 않는지, K 가 점 수보다 많을 때, 화면의 「(동점: 거리 우선)」까지 본다.
//
...
/* ================================================================
   ...
   ================================================================ */
{
    // 흩음을 가장 크게 두면 판 밖으로 나가는 점이 많다. **가장자리로 끌어오면** 그 점들이 정확히
    // 10px 선 위에 한 줄로 쌓인다 — 연속 분포에서 같은 값이 여럿 나올 수는 없으므로 센다.
    const 원래난수 = sim.window.Math.random;
    let 씨2 = 20260926;
    sim.window.Math.random = () => (씨2 = (씨2 * 1103515245 + 12345) % 2147483648) / 2147483648;
    for (const 변 of [300, 600]) {
        P(`dataManager.generateRandom(${변}, ${변}, 450)`);
        const 뽑힌점 = P('dataManager.points');
        const 가장자리 = 뽑힌점.filter((p) => [p.rx, p.ry].some((v) => Math.abs(v * 변 - 10) < 1e-6 || Math.abs(v * 변 - (변 - 10)) < 1e-6));
        if (가장자리.length > 0) bad(`무작위 자료(판 ${변}, 흩음 450): 점 ${가장자리.length}개가 판 가장자리 선 위에 쌓였다 — 밖으로 나간 점을 끌어왔다`);
        const 밖 = 뽑힌점.filter((p) => [p.rx, p.ry].some((v) => v * 변 < 10 - 1e-9 || v * 변 > 변 - 10 + 1e-9));
        if (밖.length) bad(`무작위 자료(판 ${변}, 흩음 450): 점 ${밖.length}개가 판 밖에 있다`);
        if (뽑힌점.length !== 90) bad(`무작위 자료(판 ${변}): 점을 ${뽑힌점.length}개 뽑았다`);
    }
    sim.window.Math.random = 원래난수;
}
...
/* ================================================================
   8. 손으로 짠 동점 — 무작위 자료에서는 동점이 나온다는 보장이 없다
   ================================================================ */
{
    // 물음점 (0.5,0.5) 에서 오른쪽으로 거리 d×0.02 에 점을 놓는다. 반 셋이면 K 가 홀수여도 동점이 난다
    const 판들 = [
        {이름: 'K=3 셋이 한 표씩', k: 3, 점: [['A', 1], ['B', 2], ['C', 3], ['B', 9], ['C', 10]], 답: 'A', 동점: true},
        {이름: 'K=5 동점에 끼지 못한 반이 가장 가깝다', k: 5, 점: [['C', 1], ['A', 2], ['B', 3], ['B', 4], ['A', 5], ['C', 9]], 답: 'A', 동점: true},
        {이름: 'K=5 동점 반 안에서 가까운 쪽', k: 5, 점: [['C', 1], ['B', 2], ['A', 3], ['B', 4], ['A', 5]], 답: 'B', 동점: true},
        {이름: 'K=3 다수가 가까운 한 점을 이긴다', k: 3, 점: [['B', 1], ['A', 2], ['A', 3], ['B', 9]], 답: 'A', 동점: false},
        {이름: 'K=1', k: 1, 점: [['C', 1], ['A', 2], ['A', 3]], 답: 'C', 동점: false},
        {이름: 'K 가 점 수보다 많다', k: 5, 점: [['B', 1], ['A', 2]], 답: 'B', 동점: true},
    ];
    for (const 판 of 판들) {
        const 점 = 판.점.map(([cls, d]) => ({rx: 0.5 + d * 0.02, ry: 0.5, cls}));
        // 순서를 뒤섞어 넣는다 — 넣은 순서가 곧 거리 순서면 정렬을 빠뜨려도 통과한다
        점.reverse();
        P(`dataManager.points = ${JSON.stringify(점)}`);
        P('dataManager.setQueryPoint(0.5, 0.5)');
        P('knn.calcAllDistances()');
        P(`knn.selectKNeighbors(${판.k})`);
        const 정한반 = P('knn.doVoting()');
        const 표 = P('knn.voteCounts');
        const 합 = 표.A + 표.B + 표.C;
        if (합 !== Math.min(판.k, 점.length)) bad(`동점(${판.이름}): 표의 합이 ${합}이다`);
        if (정한반 !== 판.답) bad(`동점(${판.이름}): ${정한반} 로 정했다 — 규칙대로면 ${판.답} 다`);
        if (!!P('knn.isTie') !== 판.동점) bad(`동점(${판.이름}): 동점 여부를 ${P('knn.isTie')} 로 보았다`);
        P("phase = 'DONE'");
        P('updateVotingUI()');
        const 적힌것 = doc.getElementById('finalResultText').innerText;
        const 기대글 = `Class ${판.답}` + (판.동점 ? ' (동점: 거리 우선)' : '');
        if (적힌것 !== 기대글) bad(`동점(${판.이름}): 결과를 「${적힌것}」 로 적었다 — 「${기대글}」 이어야 한다`);
    }
}
