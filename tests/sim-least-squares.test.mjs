 ...
 *
 * 화면 글자가 성한지는 `check -- sim pages` 가 본다. 여기서 보는 것은 그 위다.
 *
...
// ── 경사 하강 학습(첫 탭) ──────────────────────────────────────────────────
// **정답은 닫힌 해로 따로 구한다** — 경사 하강과 아무 상관이 없는 풀이다. 이 절이 없던 동안
// 절편 갱신의 부호를 뒤집는 돌연변이가 살아남았다(2026-09-25) — 학습기를 아무도 보지 않았다.
{
    const model = page.window.uiController && page.window.uiController.model;
    if (!model || typeof model.trainStep !== 'function') {
        bad('경사 하강 모델(uiController.model.trainStep)을 찾지 못했다');
    } else {
        const pts = [0, 0.1, 0.25, 0.4, 0.55, 0.7, 0.85, 1].map((x, i) => ({x, y: 0.2 + 0.6 * x + (i % 2 ? 0.03 : -0.03)}));
        const n = pts.length, mx = pts.reduce((t, p) => t + p.x, 0) / n, my = pts.reduce((t, p) => t + p.y, 0) / n;
        const wantW = pts.reduce((t, p) => t + (p.x - mx) * (p.y - my), 0) / pts.reduce((t, p) => t + (p.x - mx) ** 2, 0);
        const wantB = my - wantW * mx;

        // 한 걸음이 평균제곱오차의 기울기를 따라 내려가는가 — 손으로 계산해 맞댄다.
        model.w = 0.3; model.b = -0.2; model.learningRate = 0.1;
        let gw = 0, gb = 0;
        for (const p of pts) { const e = 0.3 * p.x - 0.2 - p.y; gw += 2 * e * p.x / n; gb += 2 * e / n; }
        model.trainStep(pts);
        if (!near(model.w, 0.3 - 0.1 * gw, 1e-12) || !near(model.b, -0.2 - 0.1 * gb, 1e-12)) {
            bad(`경사 하강 한 걸음이 기울기와 다르다: (${model.w}, ${model.b}) ≠ (${0.3 - 0.1 * gw}, ${-0.2 - 0.1 * gb})`);
        }
        // 충분히 돌리면 최소제곱해에 닿는가.
        model.w = 0; model.b = 0; model.learningRate = 0.3; model.diverged = false;
        for (let k = 0; k < 20000 && !model.trainStep(pts); k++);
        if (!near(model.w, wantW, 1e-3) || !near(model.b, wantB, 1e-3)) {
            bad(`경사 하강이 최소제곱해에 닿지 않는다: (${model.w.toFixed(4)}, ${model.b.toFixed(4)}) ≠ (${wantW.toFixed(4)}, ${wantB.toFixed(4)})`);
        }
    }
}
...
test('least-squares', () => { expect(fail, '위 ✗ 줄을 볼 것').toBe(0); });
