...
/* ================================================================
   ...
   `check -- sim pages` 는 **id 가 붙은 버튼만** 눌러 보는데 알고리즘을 고르는 칩에는 id 가 없어
...
/* **그리는 동안 어떤 상자의 높이도 바뀌면 안 된다.**
   ...
   단계를 넘길 때 그림 상자가 늘었다 줄었다 하면 그 아래에 있는 버튼이 아래위로
...
for (const group of [...page.el('group-tabs').children]) {
    ...
    for (const chip of [...page.el('algo-tabs').children]) {
        ...
        if (moved) bad(`${name} — 단계를 넘기는 동안 상자 높이가 바뀌었다(${moved}). 버튼이 움직인다`);
    }
}
...
for (const group of [...page.el('group-tabs').children]) {
    ...
    if (!tally.CIRCLE || tally.CIRCLE !== SORT_N_DEFAULT) {
        bad(`힙 정렬 — 트리 노드가 ${tally.CIRCLE || 0}개다(있어야 할 것 ${SORT_N_DEFAULT}개)`);
    }
    if (!tally.LINE || tally.LINE !== SORT_N_DEFAULT - 1) {
        bad(`힙 정렬 — 트리 가지가 ${tally.LINE || 0}개다(있어야 할 것 ${SORT_N_DEFAULT - 1}개)`);
    }
    // 노드가 많으면 트리를 그리지 않는다고 알려야 한다 — 조용히 사라지면 결함으로 보인다.
    setSize(page, SORT_SIZES.length - 1);
    const big = drawn(page);
    if (big.CIRCLE) bad(`힙 정렬 — 1000개인데 트리 노드를 ${big.CIRCLE}개 그렸다`);
}
...
/* ================================================================
   ...
   **끝나는 순서가 작업량의 순서와 같아야** 한다. 처음에는 기록된 장을 하나씩
...
test('sort', () => { expect(fail, '위 ✗ 줄을 볼 것').toBe(0); });
