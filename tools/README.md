...
| 무엇 | 왜 | 언제 풀리나 |
|---|---|---|
| `font_exempt`에 적은 폴더의 **글자 크기** | 아직 손질하지 않아 지금 걸면 CI가 빨간불로 굳는다. **「시뮬레이터라서」가 아니다** — 2026-08-26에 그 예외를 없앴고, 지금 남은 것은 `simulator/ai` 하나다 | 그 폴더를 손질하면 목록에서 뺀다. **비어 있는 것이 목표다** |
