...
| `check_privacy.py` | **개인정보 처리방침이 아직 참인지.** 브라우저 저장소를 쓰는 코드가 생겼는지 · 밝히지 않은 바깥 요청이 생겼는지 · 카메라를 쓰는 페이지가 방침이 말한 그 하나인지 · 첫 화면과 시뮬레이터 입구에 방침 링크가 살아 있는지. **여기서 빨간불이 나면 코드를 되돌리거나 방침을 고치고 시행일을 새로 적는다** | 저장소·요청·카메라를 건드린 뒤. `npm run ci`가 부른다 |
...
| `check_heuristic_tree.mjs` | **정보 이용 탐색 시뮬레이터의 트리 탭을 페이지 원문 그대로 돌려** 꺼내는 규칙과 내놓은 트리를 본다 — 회차마다 「f가 가장 작은 것, 같으면 먼저 발견한 것」을 꺼내는가, h(n)이 목표까지의 칸 수 그대로이고 남은 비용을 넘지 않는가, 새 트리가 **목표가 없는 곁가지를 파고들었다 되돌아오는 장면**을 보여 주는가. 마지막 하나가 없으면 A*가 왜 헤매는지 화면에서 볼 것이 없다 | 트리 탭이나 탐색 엔진을 고친 뒤. `npm run ci`가 부른다 |
...
| `check_sims.mjs` | **`simulator/` 아래 전부**를 훑는다(갈래 폴더를 새로 만들면 따로 등록하지 않아도 딸려 온다) — 뜨는가 · 캔버스가 화면 크기 × 화면 배율로 잡히는가(전체 화면을 드나든 뒤까지) · 마크업의 `on*="…"` 가 가리키는 것이 실제로 있는가 · **마크업의 단추를 하나씩 눌러 보고** 죽지 않는가 · 눌러 본 뒤 화면 글자에 **`NaN`·`Infinity`·`undefined`** 가 섞이지 않는가. d3 를 쓰는 뷰(`tree-view`·`graph-view`)와 p5·ml5 를 가짜로 때운 페이지는 출력에 찍힌다 — **그 페이지의 통과는 「죽지 않는다」까지만 뜻한다** | 시뮬레이터를 고친 뒤. `npm run ci`가 부른다 |
...
| `check_gridworld.mjs` | **그리드월드 Q러닝을 페이지 원문 그대로 돌려** 최단 경로를 배우는지 본다 — 칸마다 −0.1 · 목표 +10 이면 최적 정책이 곧 최단 경로이므로, **너비 우선 탐색으로 따로 구한 최단 거리**와 탐욕 정책의 걸음 수를 맞댄다. 미로에 길이 있는지, 이동·보상·Q 갱신이 공식과 같은지, 화면의 화살표가 그 정책이고 다시 그려도 흔들리지 않는지 | 그리드월드를 고친 뒤. `npm run ci`가 부른다 |
| `check_vision.mjs` | **컴퓨터 비전 데모의 좌표 셈과 되돌아가는 길**을 본다. **모델은 검사할 수 없다** — ml5 와 p5 를 가짜로 때우므로 통과는 「좌표 셈과 안내가 맞다」까지만 뜻한다. 카메라 해상도가 640x480 이 아닐 때 상자와 얼굴 점을 비율대로 옮기는지, 거울 모드에서 좌우를 뒤집는지, 화면 위에 붙은 상자의 이름표를 안쪽에 넣는지, 카메라를 못 쓸 때 까닭에 맞는 말을 하고 버튼을 되돌리는지, **늦게 도착한 낡은 모델을 버리는지** | 비전 데모를 고친 뒤. `npm run ci`가 부른다 |
| `check_kmeans.mjs` | **k-평균을 페이지 원문 그대로 돌려** 두 걸음이 규칙대로인지 본다 — 걸음마다 **오차 제곱합을 직접 구해 늘어나지 않는지** 본다(모으기도 옮기기도 이 값을 늘릴 수 없는 것이 k-평균의 성질이라, 한 번이라도 늘면 둘 중 하나가 어긋난 것이다). 처음 중심을 데이터 위에 겹치지 않게 놓는지, 가장 가까운 중심을 고르는지, 옮긴 자리가 소속 점들의 평균인지, 끝난 판에서 옮길 점이 없는지 | k-평균을 고친 뒤. `npm run ci`가 부른다 |
| `check_bandit.mjs` | **여러 팔 밴딧을 페이지 원문 그대로 돌려** 평균과 탐험·활용을 본다 — 뽑은 확률이 정해 둔 구간 안이고 최고 기계의 자리가 굳지 않는지, 예측값이 받은 보상의 평균 그대로인지, ε=0/1 에서 고르는 규칙이 맞는지, 오래 돌리면 실제 최고 기계로 모이고 예측값이 실제 확률에 닿는지, 화면의 값·횟수·왕관이 실제 상태와 같은지 | 밴딧을 고친 뒤. `npm run ci`가 부른다 |
| `check_multiple_regression.mjs` | **다중 선형 회귀를 페이지 원문 그대로 돌려** 최소제곱해에 닿는지 본다 — **정답을 정규방정식으로 따로 푼다**(경사 하강과 아무 상관이 없는 풀이다). 한 걸음의 갱신, **되돌린 계수가 원래 단위의 회귀식인지**(여기가 틀리면 학습은 멀쩡한데 화면의 숫자만 거짓말을 한다), 화면의 평균제곱오차, 부분 그림의 초록 선이 「다른 변수를 평균에 둔」 선인지와 오차 선이 거기 닿는지 | 다중 선형 회귀를 고친 뒤. `npm run ci`가 부른다 |
| `check_svm.mjs` | **SVM 을 페이지 원문 그대로 돌려** 여백이 최대인지 본다 — **여백의 정답을 기하로 따로 구한다**(갈라지는 자료에서 가장 넓은 여백은 두 반의 볼록 껍질 사이 거리의 절반이다). Pegasos 한 걸음의 갱신, 다 맞히는지, 서포트 벡터가 여백 안쪽의 점들인지, ‖(w,b)‖ 가 규제 상한 안인지, **그린 선과 점선이 실제로 w·x+b = 0 과 ±1 인지** | SVM 을 고친 뒤. `npm run ci`가 부른다 |
| `check_knn.mjs` | **k-NN 을 페이지 원문 그대로 돌려** 거리를 검사가 직접 재어 맞대어 본다 — 유클리드 거리인가, 고른 k개가 실제로 가장 가까운 k개인가(거리의 목록으로 맞대므로 동점 처리에 매이지 않는다), 다수결과 동점 규칙이 맞는가, k를 바꾸면 곧바로 다시 셈하는가, **그린 굵은 선이 고른 이웃과 하나씩 맞물리는가** | k-NN 을 고친 뒤. `npm run ci`가 부른다 |
| `check_logistic.mjs` | **로지스틱 회귀를 페이지 원문 그대로 돌려** 한 걸음의 갱신을 손으로 계산해 맞댄다 — 교차 엔트로피의 기울기와 같은가, 여러 걸음 뒤 손실이 줄고 갈라지는 자료를 다 맞히는가, 화면에 적는 식이 정규화를 되돌린 식인가, 예측 확률과 결정 경계가 모형과 같은가, **학습률이 커서 손실이 오르내릴 때 그렇다고 말하는가** | 로지스틱 회귀를 고친 뒤. `npm run ci`가 부른다 |
| `check_decision_tree.mjs` | **결정 트리를 페이지 원문 그대로 돌려** 마디마다 나눌 수 있는 «모든» 자리를 따로 세어 본다 — 고른 나눔이 가중 지니 불순도를 가장 낮게 하는가, 더 나눌 수 있는데 잎으로 끝내지 않는가, 잎의 답이 그 잎에 온 데이터의 다수인가, 깊이·잎 최소 개수를 지키는가, **그린 경계선이 트리의 나눔과 맞물리고 제 칸 안에서만 그어지는가**. 어느 자리를 골랐는지가 아니라 「그보다 나은 자리가 없는지」를 따지므로 동점 처리에 매이지 않는다 | 결정 트리를 고친 뒤. `npm run ci`가 부른다 |
| `check_river.mjs` | **강 건너기를 페이지 원문 그대로 돌려** 규칙과 탐색을 따진다 — 상태가 열여섯뿐이라 **정답을 전부 세어 따로 구한다**. 안전 판정 · 이웃 상태 · 최단 일곱 수 · 위험해지는 이동을 막는지 · 상태 공간 트리의 층마다 마디와 그 종류가 너비 우선 탐색을 그대로 따라간 것과 같은지(가지치기된 마디가 화면에 남는지까지) | 강 건너기를 고친 뒤. `npm run ci`가 부른다 |
| `check_hanoi.mjs` | **하노이의 탑을 페이지 원문 그대로 돌려** 규칙과 그린 자리를 따진다 — 재귀 해법이 2ⁿ-1수이고 그대로 옮기면 다 옮겨지는가, 규칙에 어긋나는 이동이 판을 바꾸지 않는가, 자동 풀이의 회차마다 판이 해법과 같은가, 상태 공간 그래프의 상태·간선·최적 경로가 규칙대로이고 **자리가 겹치거나 선이 서로 넘나들지 않는가** | 하노이의 탑을 고친 뒤. `npm run ci`가 부른다 |
| `check_nqueen.mjs` | **N-퀸의 되짚기를 페이지 원문 그대로 돌려** 걸음을 한 번에 하나씩 맞대어 본다 — 시도·놓기·가지치기·되짚기의 차례가 규칙과 같은가, 찾은 배치가 실제로 답이고 «처음 만나는» 답인가, 센 노드 수가 시도 횟수와 같은가, 직접 놓는 판의 충돌 표시·공격 범위·힌트가 맞는가 | N-퀸을 고친 뒤. `npm run ci`가 부른다 |
| `check_wumpus.mjs` | **웜퍼스 월드를 페이지 원문 그대로 돌려** 추론을 따진다 — 뽑은 지도가 금까지 갈 수 있는가, 「안전」·「함정 확정」·「괴물 확정」이 실제와 맞는가, **지각만으로 안전을 증명할 수 있는 칸을 두고 죽지 않는가**(모순되지 않는 세계를 전부 세어 정답을 따로 구한다), 직접 걷는 탭이 벽을 뚫거나 끝난 판에서 움직이지 않는가 | 웜퍼스 월드를 고친 뒤. `npm run ci`가 부른다 |
| `gen_simulator_index.py` | **시뮬레이터만 담은 입구 `simulator/index.html`을 루트 `index.html`에서 구워 낸다.** 과목 섹션과 그 nav 링크를 빼고, 링크를 한 단 안에서 본 주소로 바꾼다. `--check`를 주면 쓰지 않고 **저장소의 파일과 같은지만** 본다 — 같은 화면을 두 벌 손으로 관리하면 한쪽이 조용히 낡기 때문이다. 덤으로 **아무 데서도 링크되지 않는 시뮬레이터**를 찾는다 | `index.html`을 고친 뒤 `npm run gen:sim-index`. 검사는 `npm run ci`가 부른다 |
...
## `check_fullscreen.mjs` — 전체 화면에서 조작이 다 되는가
...
## `check_sim_terms.py` — 시뮬레이터의 말이 교과 용어인가

`check_html.py`의 금지어 검사는 **HTML만** 읽는다. 그런데 시뮬레이터의 글은 거의 전부
...
- `check_html.py`의 `BANNED_WORDS`를 **가져다 함께 본다** — 목록을 두 벌로 두지 않는다.
