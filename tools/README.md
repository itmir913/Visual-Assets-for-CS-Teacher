...
| `_sim-harness.mjs` | 시뮬레이터 페이지를 **원문 그대로 node 에서 돌리는 받침대.** 인라인 스크립트를 떼어 내 DOM 을 흉내 낸 샌드박스에서 `load` 까지 돌린다. 캔버스는 그린 좌표를 적어 두는 가짜다. `SIM_DIR` 로 옛 판을 가리킬 수 있다 | 검사를 새로 쓸 때 받쳐 쓴다. 직접 부르지 않는다 |
| `check_sims.mjs` | **시뮬레이터 전부**를 훑는다 — 뜨는가 · 캔버스가 화면 크기 × 화면 배율로 잡히는가(전체 화면을 드나든 뒤까지) · 마크업의 `on*="…"` 가 가리키는 것이 실제로 있는가. d3·p5·ml5 를 가짜로 때운 페이지는 출력에 찍힌다 — **그 페이지의 통과는 「죽지 않는다」까지만 뜻한다** | 시뮬레이터를 고친 뒤. `npm run ci`가 부른다 |
