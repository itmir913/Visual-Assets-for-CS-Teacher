...
| `check_sim_pages.mjs` | **`simulator/` 아래 전부**를 훑는다(갈래 폴더를 새로 만들면 따로 등록하지 않아도 딸려 온다) — 뜨는가 · 캔버스가 화면 크기 × 화면 배율로 잡히는가(전체 화면을 드나든 뒤까지) · 마크업의 `on*="…"` 가 가리키는 것이 실제로 있는가 · **마크업의 단추를 하나씩 눌러 보고** 죽지 않는가 · 눌러 본 뒤 화면 글자에 **`NaN`·`Infinity`·`undefined`** 가 섞이지 않는가. d3 를 쓰는 뷰(`tree-view`·`graph-view`)와 p5·ml5 를 가짜로 때운 페이지는 출력에 찍힌다 — **그 페이지의 통과는 「죽지 않는다」까지만 뜻한다** | 시뮬레이터를 고친 뒤. `npm run ci`가 부른다 |
