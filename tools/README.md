...
| 파일 | 용도 | 언제 쓰나 |
|---|---|---|
| `run.mjs` | **검사·감사 러너.** 목록(`CHECKS`·`SIMS`·`AUDITS`)이 여기에만 있다. 이름을 테스트 파일로 바꾸어 Vitest 에 넘기고, 이름 뒤의 인자는 `CHECK_ARGS`로 건넨다. 시뮬레이터 동작 검사는 `check -- sim [이름…]`으로 부른다. 이름을 주면 그것만, 안 주면 산출물 검사를 뺀 전부를 돌고 실패한 것을 끝에 모은다 | `npm run check` · `npm run audit -- <이름>`으로 부른다 |
