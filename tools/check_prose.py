...
VERBS = [
    ...
    (H + r"(?:갈라\s?(?:주|줍|줘|준|보|볼|봅|내|낸|냈|낼|냅|놓|놔|놨|놀|놉|두|둔|둘|둡|뒀)|가른|가를|가르[는고며면]|가릅|갈랐)",
     "가르다", "구분하다 · 나누다",
     ["갈라 줍니다", "가르는 기준", "가른다", "가릅니다", "무엇이 갈랐을까", "갈라 보기",
      "승부를 가른 것", "둘을 가를 때",
      "갈라 낼 수", "갈라놓을 수", "갈라 놓는 일", "갈라 두었기"],
     ["가르치는", "가르쳐", "갈라지는", "갈라졌다"]),
    (H + r"(?:가릴\s?수|가려\s?(?:내|낸|낼|냅|보(?!이)|봅|봐|봤|볼|본))", "가리다(구별)", "구별하다 · 판단하다",
     ["가릴 수 있을까", "가려낸다", "가려 보는 것", "가려봅시다"], ["가려진", "가리키는", "가려 보이지"]),
