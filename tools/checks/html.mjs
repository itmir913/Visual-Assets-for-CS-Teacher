...
//     npm run check -- html 인공지능기초/1-1-2.….html  # 인자만 검사. 글롭을 여럿 이어도 된다(Git Bash 에서 — PowerShell · cmd 는 글롭을 전개하지 않는다)
//     npm run check -- html -v                        # 전체를 돌면서 항목별 결과까지
//
// 위반이 하나라도 있으면 실패한다(경고는 통과). 경로는 현재 폴더 → 저장소 루트 순으로 찾는다.
// **`확인 필요`는 CI 를 막지 않지만 0 을 유지한다** — 쌓이면 새로 생긴 것이 묻힌다.
//
// **빠지는 것** — `subjects.json` 의 `font_exempt` 폴더의 글자 크기와 `fs-figure` 표시를 단
// `font-size`(그림 «안»의 글자)는 늘 뺀다. `REWRITE_PENDING` 폴더는 인자 없이 전체를 돌 때만 뺀다.
//
// **못 잡는 것** — 375px 실측에서만 보인다: 섹션이 6화면을 넘는지, `table-prose` 를 flex 자식에
// 넣고 `min-w-0` 을 빠뜨린 가로 넘침, `table-prose` 를 붙여야 하는 표인지. 재는 법은
// `docs/수정-레시피.md`. 실측은 사용자가 지시할 때만 한다(CLAUDE.md).
//
...
// 견준·견줍·견줘·견줌으로 갈라지므로 끝 음절을 묶어 준다. 그 자리에 쓸 말도 함께 넣는다.
// **뜻이 다른 동음이의어는 정규식에서 걸러 내고, 왜 거르는지 한 줄 주석을 옆에 남긴다** —
// 없으면 다음 사람이 구멍으로 읽는다.
//
