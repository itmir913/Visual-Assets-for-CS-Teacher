...
const VERBS = [
    ...
    ['펼(?:치|쳐|쳤|칠|친|침|칩)|' + H + '(?:펴[다고서며면는지도야기려]|폈|폅)', '펼치다 · 펴다', '확장하다 · 전개하다 · 나열하다',  // verbs: 예시
        ['펼쳤지만', '넓게 펼쳤다가', '한 줄로 펴면', '폈다', '펼칩니다', '펴는', '폅니다'], ['살펴보다', '살펴서', '책을 펴낸 곳']],  // verbs: 예시
    ['들여다\\s?(?:보|봅|봐|봤|볼|본)', '들여다보다', '살펴보다',
        ['들여다봅시다', '들여다보면', '들여다 봐야'], []],
    [H + '손(?:보|봅|봐|봤|볼|본)', '손보다', '정리하다 · 점검하다 · 수정하다',
        ['손보는 일', '미리 손봅니다', '손봐야', '손본다', '손볼 곳', '손보던'], ['손 보호']],
    // 「따지지 않」은 시뮬레이터에서 새어 나간 꼴이다.
    // 「따지므로 · 따지던」은 어미를 한 글자씩 적은 목록에서 새어 나간 꼴이다 — 「따지」 뒤는 다 막는다.
    [H + '따(?:지(?!막)|져|졌|질|진|집)', '따지다', '비교하다 · 검토하다',
        ['따져 볼', '따지면', '따집니다', '순서를 따지지 않고', '크기만 따지므로', '729가지를 따지던', '차례를 따진다', '따질 것', '따진 결과'], ['따지막']],
    // 「솎다 · 돌리다(실행) · 뒤지지 않고」는 시뮬레이터 정제(2026-09-24)에서 물러났다.
    [H + '솎', '솎다', '건너뛰다 · 제외하다',
        ['가지를 솎아 내면', '솎는다', '솎습니다', '솎이지 않는다', '솎은', '솎을'], []],
    ...
    ['뒤져(?:서|도|\\s?(?:보|찾))|[을를]\\s(?:다\\s|전부\\s|모두\\s|다시\\s)?뒤(?:지[는고게며면다]|졌|진|질)|뒤지지\\s?않(?!는)',
        '뒤지다', '찾아보다 · 탐색하다',
        ['뒤져서 찾아봄', '뒤져 보면', '전부 뒤지지 않고', '데이터를 뒤져도', '엉뚱한 자리를 뒤지게', '자리를 뒤지는 셈', '창고를 뒤진다', '서랍을 뒤질'],
    ...
    ['흩뿌(?:리|려|렸|린|릴|림|립)', '흩뿌리다', '흩어 놓다',
        ['흩뿌려 보세요', '흩뿌린 데이터', '흩뿌려짐', '흩뿌립니다'], ['흩어 놓은']],
    ...
    [H + '넘나(?:들|드|든|듭)', '넘나들다', '오가다',
        ['과목을 넘나들며', '경계를 넘나드는', '넘나듭니다', '넘나든'], []],
    ...
    // 2026-09-26 사용자 결정으로 물러난 여섯. 「뒤바뀌다 · 휘둘리다 · 결론짓다」는 같은 자리에서 검토하고 그대로 쓰기로 했다.
    // 자동사 「부풀다(부풀었다 · 부풀어 오르다)」는 막지 않는다 — 물러난 것은 사실보다 크게 적는 「부풀리다」다.
    ['부풀(?:리|린|릴|립|려|렸|림)', '부풀리다', '과장하다 · 과장되다',
        ['부풀리지 않기', '부풀려 적은', '부풀린 쪽', '부풀렸다', '부풀릴 수', '부풀립니다', '성능이 부풀려진다'],
        ['기대가 부풀었다가', '부풀어 오르는', '부분']],
    // 기술이 차례로 이어지면 연결되다, 모양이 맞는 것끼리 붙으면 결합되다.
    ['맞물', '맞물리다', '연결되다 · 결합되다',
        ['맞물려 돌아가기', '맞물리는지', '맞물린 모양', '맞물렸다', '맞물릴'], ['맞추어', '물려받은']],
    [H + '본(?:뜨|떠|떴|뜬|뜰|뜸|뜹)', '본뜨다', '모방하다',
        ['모델 = 본뜬 것', '모양을 본떠 만든', '본떴다', '본뜨면', '본뜰', '본뜹니다'], ['기본 뜻', '본뜻', '견본을 떠올리면']],
    [H + '겨(?:루|뤄|뤘|룬|룰|룸|룹)', '겨루다', '경쟁하다 · 비교하다',
        ['서로 겨루는 사이', '겨룰 상대', '자주 겨룬다', '겨뤄 보면', '겨뤘다', '겨룹니다'], ['겨우', '겨울', '겨레']],
    // 「이끌어내다」는 그대로 쓴다(인공지능기초 1-2-4 논리 추론). 이끌어내기 어색하면 도출하다.
    ['(?<!이)끌어\\s?(?:내|낸|냈|낼|냅|냄)', '끌어내다', '이끌어내다 · 도출하다',
        ['무엇을 끌어낼 것인가', '장치를 끌어낼 수', '끌어낸다', '끌어 내면', '끌어냈다'],
        ['결론을 이끌어내는', '어떻게 이끌어낼까', '이끌어낸 규칙', '이끌어 낸']],
    ['곁들', '곁들이다', '덧붙이다 · 보조(상자 이름)',
        ['곁들이 설명', '곁들여', '곁들인 그림', '곁들일 때만', '곁들였다'], ['곁에 둔']],
];
...
const PATTERNS = [
    ...
    // 기관명이 바뀌었다(통계청 → 국가데이터처). 옛 이름은 「옛 통계청」으로 덧붙일 때만 쓴다.
...
// 「vs」 규칙이 한글을 찾는 범위 — 한 줄 안에서, 코드가 아닌 태그는 넘는다.
const VS_CTX = '(?:[^<>\\n]|<(?!/?(?:code|pre|kbd|samp)(?!\\p{L}))[^<>\\n]*>)';
const STYLE_NOW = [
    ...
    ['VS Code', '(수업에서 원리를 확인하려고 지어낸 예)', '(수업용 예)</p>', '(예: 로봇 팔, 스피커)',
        '사례: 가', '해 보기: 주제']],
    // 기준서 12 의 소문자 꼴 — 「A vs B · vs. · Vs」(2026-09-26 사용자 지시). 한글 문장 속만 막는다 —
    // 같은 줄 앞뒤 예순 글자 안에 한글이 없으면 영어 원문(「clustering vs association」)이다.
    // 코드(<code> · <pre> 속)는 뺀다 — 그래서 태그를 벗긴 본문이 아니라 원문에만 건다(`RAW_ONLY`).
    // 주소 · 앵커 · 식별자(`#vs` · `/vs/` · `canvas`)는 앞뒤 글자로 거른다.
    [`(?<!<(?:code|pre|kbd|samp)(?!\\p{L})[^>]*>[^<]*)(?<![\\p{L}\\p{N}_/#.'"=&+-])(?:vs|Vs|vS)\\.?(?![\\p{L}\\p{N}_/=+-])` +
        `(?:(?<=[가-힣]${VS_CTX}{0,60})|(?=${VS_CTX}{0,60}[가-힣]))`,
        '「vs」 비교 표기', 'A와 B · A 대 B · A와 B 비교',
        ['정형 데이터 vs 비정형 데이터', 'DFS vs BFS 비교', '<strong>산도가 높습니다</strong>(0.37 vs 0.25).',
            '맹목적 3개 vs. 정보 이용 2개', '회귀 Vs 분류', 'title="A vs B 비교"'],
        ['clustering vs association', '<a href="#vs">분류와의 차이</a>', '<section id="vs" class="a">',
            '분류와의 차이 <code>a vs b</code>', '<pre>\n점수 a vs b\n</pre>', 'https://example.com/vs/비교', 'canvas 비교', 'VS Code']],
    [`(?<![&\\p{L}\\p{N}_/#"'])#[가-힣]`, '해시태그', '낱말을 가운뎃점으로 잇는다',
        ['#얼굴인식 #음성인식'], ['href="#퀴즈"', "'#결과'", '&#8251;']],
];

// ── 화면 문구의 느낌표 · 보조 용언 붙여 쓰기 ─────────────────────────────────────
// 인공지능 시뮬레이터 화면 문구 정제(2026-09-26)에서 한꺼번에 고친 두 가지다. 시뮬레이터에서
// 찾았지만 같은 꼴이 강의노트 · 배부 양식 생성기에도 있어 함께 고치고 모두에 건다.
// - 느낌표 — 상태 줄 · 알림 · 제목을 「성공! · 도전!」으로 들뜨게 쓰지 않는다. **인용한 말
//   「고양이네!」 속은 둔다** — 그 뒤에 「 없이 」가 먼저 오면 따옴표 안이다.
// - 보조 용언 — 「해보세요 · 눌러주세요 · 적어둔」은 띄어 쓴다(해 보세요 · 눌러 주세요 · 적어 둔).
//   한 낱말로 굳은 것(도와주다 · 알아보다 · 돌려주다 · 잃어버리다 …)은 앞말 목록으로 뺀다.
//   코드 파일 이름(`찍어보기.py`)은 이름이라 뺀다.
const AUX_HEAD = '(?:해|써|(?<=[가-힣])(?:어|아|여|워|와|러|려|라|져|켜|뤄|혀|춰|꿔|겨|쳐|봐|줘|둬|내|놓아))';
const AUX_WORD = '(?<!(?<![가-힣])들어)(?<!도와|알아|돌아|되돌아|뒤돌아|눈여겨|되돌려|찾아|지켜|바라|쳐다|내려다|둘러|물어|살펴|여겨|놓아|빌려|떠|엿|몰라|거들어|가려|돌려|넘겨|노려|몰아|잊어|잃어|훔쳐)';
const AUX_TAIL = '(?:보(?:세요|십시오|자|면|며|고(?!서)|기|는|았|아|시|려|지)|봐|봤|봅|볼|본|' +
    '주(?:세요|십시오|면|며|고|는|기|어|었|시|려|자|지)|줘|줬|줍|줄|준|' +
    '두(?:세요|십시오|면|며|고|는|기|었|시|려|자|지)|둬|뒀|둡|둘|둔|버(?:리|렸|려|린|릴|립))' +
    '(?![\\p{L}\\p{N}_-]*\\.(?:py|c|js)(?!\\p{L}))';
const STYLE_SCREEN = [
    ['(?<=[가-힣)」])!(?![^「」]{0,300}」)', '느낌표', '마침표로 맺거나 빼고, 인용한 말 속에만',
        ['성공입니다!', '🎉 목표 도달! (3번)', '8 × 8 (도전!)', '충돌 발생! 빨간 퀸', '「노드 추적」으로 두세요!'],
        ['「이건 사과네!」, 「저건 고양이네!」라고', '「어? 처음\n몇 번은 꽝이었어요!」', 'class="!p-4 md:!p-6"',
            'if (!ok)', '<!DOCTYPE html>', 'a !== b']],
    [AUX_HEAD + AUX_WORD + AUX_TAIL, '보조 용언 붙여 씀', '해 보세요 · 눌러 주세요 · 적어 둔 — 띄어 쓴다',
        ['퀸을 배치해보세요', '버튼을 눌러주세요', '2개 이상 넣어주세요', '미리 설정해둔', '찍어보세요', '충돌을 알려줍니다',
            '사진을 보여주며', '통째로 잘라버리자', '토론해볼까요', '체험해봅니다', '직접 해보기', '계산해두면', '외워버립니다', '표를 만들어보세요'],
        ['도와주세요', '알아보기', '살펴보면', '값을 돌려준다', '인자를 넘겨주는', '찾아보세요', '물어보면', '아주 크다',
            '단어보다 길다', '보고서', '어두운 곳', '잃어버린', '훔쳐보지', '되돌아보기', '바라보는', '해 보세요',
            'data-src="code/3-2-8.찍어보기.py"', '뒤돌아보면', '되돌려주는', '부탁을 들어주면']],
    // 「-고 + 보다」도 보조 용언이다(알고 보니 · 먹고 보니 · 해 놓고 보니, 2026-09-26 사용자 지시).
    // 「보다」 · 「보단」은 조사(「최고보다」)와 겹쳐 뺀다. 「원고본」은 한 낱말이다.
    ['(?<=[가-힣])(?<!원)고(?:보(?:니|면|며|자|세요|십시오|았|아|려|지|기|는|고)|봐|봤|봅|볼|본)' +
        '(?![\\p{L}\\p{N}_-]*\\.(?:py|c|js)(?!\\p{L}))',
    '「-고 보다」 붙여 씀', '알고 보니 · 먹고 보니 · 해 놓고 보니 — 띄어 쓴다',
        ['알고보니', '먹고보니', '해놓고보니', '듣고보면', '적어 놓고봤더니', '읽고봐야', '써 두고볼'],
...
// 원문(태그째)에만 거는 규칙. 태그를 벗긴 본문에서는 <code> 속 글이 한글 문장에 섞여 코드를 가려낼 수 없다.
const RAW_ONLY = new Set(['「vs」 비교 표기']);
