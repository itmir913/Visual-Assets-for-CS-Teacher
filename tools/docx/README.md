...
그래서 `checks/dist.mjs`의 `UNLINKED_OK`에 적어 **검사가 지키게 했다.**
그 표는 두 방향으로 작동한다.
...
**⑤ 생성기의 문장도 강의노트와 같은 문체 검사를 받는다.** 문자열이 곧 학생이 받는 문장이라
`npm run check -- prose`가 `make/` 아래 `.js`의 문자열을 본다. 범위와 빼는 칸은
[`checks/prose.mjs`](../checks/prose.mjs) 머리에 있다.
