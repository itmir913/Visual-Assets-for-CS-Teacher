...
// 문항 — 지시문 · 체크리스트
//
// 이 표의 지시문(prompt)과 체크리스트는 화면판(`인공지능기초/실습/*.html`)의 `.q-header` ·
// `.checklist-box`와 **같아야 한다.** 학생이 받는 종이와 화면이 다르면 어느 쪽이 맞는지 알 수
// 없다. `npm run check -- form-sync`가 이 표를 읽어 두 쪽을 맞대 본다.
//
//   prompt     지시문. 문자열은 보통 글씨, {b: '…'}는 굵은 글씨. 줄바꿈은 '\n'.
//              문항 7은 두 칸이라 left · right로 나뉜다(화면에서는 이어 적는다).
//   note       종이에만 있는 작은 안내(예시 · 캡처 단축키). 화면과 맞대지 않는다.
//   checklist  자기점검 체크리스트 항목.
//   answerH    답 칸의 최소 높이 [예시 보고서, 빈 양식].
// ════════════════════════════════════════════════════════════════════════════

const CAPTURE = "〈화면 캡처 단축키: 윈도우 + Shift + S〉";

const QUESTIONS = [
    {
        prompt: ["실생활이나 학문 분야에서 인공지능으로 해결하고자 하는 문제를 구체적으로 정의하시오. 어떤 상황에서 어떤 불편함 또는 필요가 발생하는지를 포함하여 서술하시오."],
        note: "(예: 갤러리에 무작위로 저장된 반려동물 사진을 일일이 손으로 분류해야 하는 불편함을 해소하기 위해, 사진을 자동으로 분류하는 시스템을 만들고자 한다.)",
        checklist: [
    ...
    {
        prompt: [
            "구현하려는 인공지능 모델을 설명하시오. 이 모델은 어떤 데이터를 ",
            {b: "입력"},
            "받아, 어떤 결과를 ",
            {b: "출력"},
            "하는가? 입력과 출력을 중심으로 모델의 역할을 구체적으로 서술하시오.",
        ],
        note: "(예: 입력 — 강아지·고양이 사진  /  출력 — 「강아지」 또는 「고양이」로 분류된 레이블)",
        checklist: [
    ...
    {
        prompt: [
            "위에서 구상한 모델을 구현하기 위해 아래 ①, ② 순서대로 선택하고 그 이유를 서술하시오.",
            {b: "\n\n①  기계학습 유형:  "},
            "지도학습과 비지도학습 중 이 모델에 적합한 유형을 선택하고, 분류·군집·예측 등의 개념을 활용하여 그 이유를 설명하시오.",
            {b: "\n\n②  구현 도구:  "},
            "①에서 선택한 유형을 구현하기 위해 티처블머신·코랩·오렌지 중 어떤 도구를 선택했는지 밝히고, 그 이유를 서술하시오.",
        ],
        checklist: [
    ...
    {
        prompt: ["모델 학습에 사용한 데이터를 설명하시오. 데이터의 출처·유형·수량·특성 등을 작성하고, 필요한 경우 스크린샷을 첨부하시오."],
        note: CAPTURE,
        checklist: [
    ...
    {
        prompt: ["수집한 데이터로 모델을 학습시킨 결과를 서술하시오. 학습이 완료된 화면 또는 학습 결과를 스크린샷으로 첨부하고, 수치나 지표를 포함하여 결과의 의미를 간략히 설명하시오."],
        note: CAPTURE,
        checklist: [
    ...
    {
        prompt: ["학습된 모델에 새로운 데이터를 직접 입력하여 성능을 테스트하시오. 테스트 전 예상했던 결과와 실제 결과를 비교하고, 차이가 발생했다면 그 원인을 분석하시오."],
        checklist: [
    ...
    {
        left: [{b: "기대 효과:  "}, "이 모델을 자신의 관심 분야(예: 의료·환경·스포츠 등)에 적용한다면 어떤 효과를 기대할 수 있는지 서술하시오."],
        right: [{b: "개선 사항:  "}, "현재 모델의 한계를 바탕으로, 성능을 향상시키기 위해 필요한 개선사항을 구체적으로 서술하시오."],
        checklist: [
    ...
    {
        prompt: ["기계학습 모델을 직접 구현하는 과정에서 달라진 생각, 어려웠던 점, 새롭게 알게 된 것, 또는 느낀 점을 자유롭게 서술하시오."],
        checklist: [
...
/** 문항 i(0부터)의 지시문을 화면과 맞댈 글로. 공백은 하나로 줄인다. 문항 7은 두 칸을 잇는다. */
const promptText = (i) => {
    const q = QUESTIONS[i];
    const segs = q.prompt || [...q.left, " ", ...q.right];
    return segs.map((s) => (typeof s === 'string' ? s : s.b)).join('').replace(/\s+/g, ' ').trim();
};

// 지시문 조각 → TextRun[]
const promptRuns = (segs) => segs.map((s) => (typeof s === 'string' ? run(s) : run(s.b, {bold: true})));

const noteRuns = (note) =>
    note ? [run("\n\n" + note, {size: FONT_SMALL, color: "555555", italics: true})] : [];
...
const makeQuestion = (i, c) => {
    const q = QUESTIONS[i];
    const n = i + 1;
    const h = c ? q.answerH[0] : q.answerH[1];
    const table = q.left
        ? questionTable2Col(
            promptRuns(q.left),
            promptRuns(q.right),
        ...
        : questionTable(
            [run(`문항 ${n}.  `, {bold: true}), ...promptRuns(q.prompt), ...noteRuns(q.note)],
            c ? toParagraphs(c[`q${n}`]) : [p("")],
            h
        );
    const head = n === 1
        ? [
        ...
        : [sectionTitle(`문항 ${n}`)];
    return [...head, table, ...emptyP(1), checklistBox(q.checklist)];
};
        ...
        sections: [
            {
                ...
                children: [
                    ...QUESTIONS.flatMap((_, i) => [...(i ? [pageBreak()] : []), ...makeQuestion(i, c)]),
                ],
            },
        ],
    });
...
module.exports = {makeDocument, QUESTIONS, promptText};
