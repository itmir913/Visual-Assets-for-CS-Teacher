...
//
// 남기는 것 — `head` · `style` · `script` 는 버리고, SVG 는 `aria-label` 만 `[그림] …` 으로,
// `<summary>` 는 `[접기] …` 로 남긴다. `--digest` 는 학습 목표 · 절 제목 · 정의꼴 문장 · 핵심 정리 ·
// 퀴즈만 남긴 요약판이다 — 파일 사이의 어긋남을 찾게 할 때 전문판과 함께 넘기고, 지적은
// 전문판에서 원문을 확인한 것만 받는다. 크기는 `--digest --stats` 로 잰다.
//
