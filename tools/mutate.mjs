...
const MUTANTS = [
    ...
    ['sim-ordering', 'src/entries/_lib/sort/algo/bubble.js', '${end + 1}번째 자리가 확정', '${end}번째 자리가 확정', '버블 정렬 설명의 자리 번호 하나 어긋남'],
    ...
    ['sim-ordering', 'src/entries/_lib/sort/algo/quick.js', '`피벗이 ${wall}번 자리에', '`피벗이 ${wall + 1}번 자리에', '퀵: 설명의 피벗 자리 어긋남'],
