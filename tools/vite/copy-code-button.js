...
//
// 클릭 처리는 **위임 리스너 하나**다. 블록마다 id를 붙이고 `onclick="copyCode('code-98')"`을
// 쓰던 방식은 쓰지 않는다 — id가 파일을 넘어 95개나 중복돼 있었고, 코드가 실파일로 빠지면
// id를 붙일 자리도 없다.
//
// **클래스 이름은 Tailwind 유틸리티가 아니다.** 버튼 마크업이 소스 HTML에 없어 Tailwind가
// 스캔할 수 없으므로, 모양은 `src/styles/_code-copy.css`의 보통 CSS가 준다.
